...
\begin{tikzpicture}[
  ...
  % Useful sub-tile geometry: the highlighted cell of the 4x4 grid
  % spans x in [0, 0.5], z in [0, 0.5] (one near-centre sub-tile).
  \pgfmathsetmacro{\ttileLo}{0.0}
  \pgfmathsetmacro{\ttileHi}{0.5}
  ...
  % Highlighted sphere sub-tile: the gnomonic image of the cube cell
  % x in [0, 0.5], z in [0, 0.5].  Drawn in TileLine to pair it with
  % the highlighted cube sub-tile of stage 8.
  % Edge at x = 0, z in [0, 0.5]
  \draw[TileLine, very thick]
    plot[domain=0:0.5, samples=16, smooth]
      ({0}, {-1/sqrt(1+\x*\x)}, {\x/sqrt(1+\x*\x)});
  % Edge at x = 0.5, z in [0, 0.5]
  \draw[TileLine, very thick]
    plot[domain=0:0.5, samples=16, smooth]
      ({0.5/sqrt(0.25+1+\x*\x)}, {-1/sqrt(0.25+1+\x*\x)},
       {\x/sqrt(0.25+1+\x*\x)});
  % Edge at z = 0, x in [0, 0.5]
  \draw[TileLine, very thick]
    plot[domain=0:0.5, samples=16, smooth]
      ({\x/sqrt(\x*\x+1)}, {-1/sqrt(\x*\x+1)}, {0});
  % Edge at z = 0.5, x in [0, 0.5]
  \draw[TileLine, very thick]
    plot[domain=0:0.5, samples=16, smooth]
      ({\x/sqrt(\x*\x+1+0.25)}, {-1/sqrt(\x*\x+1+0.25)},
       {0.5/sqrt(\x*\x+1+0.25)});
  ...
  % Highlighted sub-tile (cube cell x in [0,0.5], z in [0,0.5])
