...
**Three-level hierarchy**: long BHL papers (2101.02645, 2309.14783) use a consistent three-level structure: section → subsection → named paragraph. The reader navigates at the section level, reads at the subsection level, and absorbs detail at the named-paragraph level. Named paragraphs within a subsection run 3–10 sentences in the actual corpus, though 4–6 sentences is the modal target. This creates the characteristic visual rhythm of BHL writing: a sequence of named moves, each self-contained, separated by bold headings, with no undifferentiated prose block running more than ten sentences without a paragraph break.

Letters (2406.12826, 2311.11790) push this to the extreme: they have no subsections at all, only named paragraphs with descriptive titles (e.g., "Introduction to torsion", "No-go theorem for vector torsion", "In this letter"). Everything from the introduction through the conclusions is delivered via paragraph headers. This is only viable because the headers do real organisational work.

**Paragraph length norm**: BHL paragraphs — whether named or unnamed — consistently run 3–8 sentences; count on the actual high end reaching 10 in rare cases. A one-sentence paragraph signals a transition, not a finding. A paragraph exceeding 12 lines almost always contains two separate points; split it. For the MSci report, aim for 5–6 sentences per named paragraph in the theory sections and 4–5 in the results section.
...
The primary transition device in BHL writing is **"However,"** at the start of a sentence. It appears frequently — in a 3-page letter (2311.11790) it appears four times; in a full PGT paper it appears five or more times in the introduction alone. "However," signals a turning point between a prior claim and a complication, a prior attempt and its limitation, or a positive finding and a caveat. It is the canonical BHL connector and should be used without hesitation when the argument turns.

Transitions of the "furthermore / in addition" kind are rare. The typical BHL move is the last sentence of a paragraph announcing the next move, and the next paragraph's opening sentence executing it. The PGT conformal paper (2406.12826) transitions from the scale-invariant toy model to the gravitational application with a "Recap and extension" paragraph that uses a numbered list to collect the three prior observations, ending: "We will now connect these three observations by showing that eWGT is the unique scale-invariant embedding of PGT." The next paragraph then delivers exactly that. The connector is the sentence "We will now connect…", which is both a transition and a commitment.
...
**Notation-declaration paragraph** (typically the first named paragraph in §2, or appearing after the conventions note at the end of §1): BHL papers include a notation-declaration block specifying: metric signature, index ranges (e.g., Greek holonomic/coordinate indices vs. Roman Lorentz/frame indices), natural units ($\hbar = c = 1$), and any non-standard abbreviations. In 2101.02645 the notation declaration appears *after* the physics setup in a "Following the conventions of..." note, not as a dedicated opening paragraph. For the TIDAL report, include a brief conventions sentence at the end of the Introduction or as the first named paragraph of §2, declaring: $(+,-,-,-)$ signature, $\kappa^2 = 16\pi G$, uppercase Roman for Lorentz frame indices, lowercase Greek for coordinate indices, and natural units. **Do not** include the irreducible torsion decomposition labelling here — the decomposition is introduced as a named paragraph *after* the general quadratic action (see coupling constant introduction convention below), not in the conventions block. The conventions paragraph is for index notation, signature, and units only.
...
## (m) Additional BHL Conventions and Terminology

**"Sector" language**: BHL papers use "sector" freely and naturally for both conceptual domains (the matter sector, the gravitational sector) and technical decompositions (the spin-parity $J^P$ sectors, the trace sector, the axial sector). For the TIDAL report, "torsion sector", "trace-channel sector", "hx sector", "ax sector" are all correct BHL terminology. Never avoid "sector" in favour of more circumscribed language like "component" or "contribution."

**Units declared early**: Natural units ($\hbar = c = 1$) are declared explicitly near the start of the theory section, typically in the first sentence introducing the Lagrangian or in the conventions paragraph. Example from 2311.11790: "...setting $\hbar \equiv c \equiv 1$..." — the declaration is in-line, not a separate sentence. Do not leave units implicit.

**Subject-first assertion in solo-author papers**: Barker's solo-authored papers consistently begin paragraphs with the mathematical object or assertion before citing supporting literature. The pattern is: "The basic units of the particle-physics formulation of PGT are [definition]..." (then citation, if needed). Joint papers sometimes lead with citation: "Jones showed [cite] that X...". For solo-author voice, lead with the physics, not the citation.

**Explicit principle-naming**: Barker's solo papers name the conceptual architecture explicitly — "This principle of selective non-interference..." (2205.13534). When a methodology choice is made, give it a memorable name or phrase. This is not jargon inflation; it aids referee recall and makes the paper's logical moves stick.

**Conclusion variation by result type**: Conclusions do not always open with "In this paper we have shown..." The opening varies with the character of the result:
- *For positive/clear results*: "We have shown that..." or "In this paper we have presented..."
- *For results that complicate prior work*: "This paper has sought to demonstrate [effect]..." (2303.11094) — softer framing
- *For negative/inconclusive results*: "The purpose of this paper was modest" (2407.09598) — explicit hedging before the findings summary
- *For short letters*: A named paragraph "Closing remarks" with a philosophical opening sentence

For the TIDAL report — which has positive validation, null results, and a positive structural finding — use "We have shown that..." for the positive result and switch to "However," to introduce the null results' implications.

**"As expected" usage**: The phrase "as expected" appears sparingly in BHL papers (once per paper, if at all) to confirm that a known prediction is reproduced. Use it only when the TIDAL simulation reproduces the Boccaletti formula — "the simulated conversion probability reproduces the analytic formula, as expected." Do not use it for the null results.
