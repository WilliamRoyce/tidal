...
\begin{tikzpicture}[
    ...
    % Neutron star: soft sphere -- an OFFSET radial palette gradient (no white
    % highlight or black rim, unlike `ball color`), so it matches the soft corona
    % and field-line tones rather than standing out. The bright tone sits up-left
    % (gentle illumination) and deepens to a teal toward the lower-right edge.
    \begin{scope}
        \clip (0,0) circle (\E);
        \shade[inner color=CamBlue, outer color=CamDarkBlue!70!CamWarmBlue]
              (-0.32*\E,0.32*\E) circle (1.5*\E);
    \end{scope}
    \draw[very thin, Corona] (0,0) circle (\E);

    \node[CamDarkBlue] at (0.203*\xmax,0.87*\ymax) {\contour{CamLightBlue}{Magnetosphere}};
    \node[CamDarkBlue] at (0.24*\xmax,-0.315*\ymax) {\contour{CamLightBlue}{Neutron star}};

    \foreach \i[evaluate={\y=\i*0.2*\ymax;}] in {-1,0,1}{
        \draw[gw] (-0.9*\xmax,\y) -- (-2*\E,\y);
    }
    \node[GW] at (-0.65*\xmax,0.4*\ymax) {\contour{CamLightBlue}{Gravitational waves}};

    \coordinate (EM) at (2*\E,0);
    \foreach \i [evaluate={\r={\i*0.57*\xmax/\N};}] in {1,...,\N}{
        \draw[line width=2.5pt, CamLightBlue] ($(EM)+(+\cone:{\r} and {0.5*\r})$) arc(+\cone:-\cone:{\r} and {0.5*\r});
    ...
    \node[Photon,align=center] at (0.81*\xmax,0.80*\ymax) {\contour{CamLightBlue}{Photon} \\ \contour{CamLightBlue}{(Fast Radio Burst)}};
\end{tikzpicture}
