...
\paragraph*{From the survey rankings to physical magnitudes}
The survey reports the structure of where amplification lives in
coupling space, not concrete amplification numbers for a specific
astrophysical source. The amplification score $A(\theta)$
of~\cref{AmplificationScore} grows exponentially in $t_\text{end}$
on the unbounded plane-wave BGMF for any sector with
positive-real-part eigenvalues, so its numerical value at any one
$(B_0, t_\text{end})$ pair has no standalone meaning. The two
outputs the survey delivers are the operators that carry the
amplification and the coupling-space regions in which they sit.
The finite-extent follow-up identified above turns those into a
conversion probability at a candidate astrophysical source --- a
chosen $\Bzero$ profile and gravitational-wave frequency band ---
by bounding the eigenvalue-driven exponential to the finite
interaction length; the operators that rank highest across this
survey ($\chi_1$, $\Del{1}$, $\Bet{1}$, $d_{21}$, $\Del{m}$) are
the inputs that calculation needs.
