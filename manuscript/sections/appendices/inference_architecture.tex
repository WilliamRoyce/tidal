...
\input{sections/listings/d_kl_methods_appendix.tex}

\begin{table*}[t]
  \centering
  \caption{Per-theory $D_\mathrm{KL}$ carrier identification for the
    plane-wave-benchmark survey.  Each ranked sub-table lists the top-$k$
    ...
    distinguishing the two posteriors.  Generated by
    \texttt{scripts/analysis/extract\_d\_kl.py} from the
    \texttt{parameter\_importance.json} written by
    \texttt{recompute\_parameter\_kl.py}; re-running the pipeline picks
    up new chains (e.g. forthcoming localised-geometry runs) without
    edits.}%
  \label{KLPerTheory}
  \input{sections/listings/d_kl_per_theory.tex}
\end{table*}
