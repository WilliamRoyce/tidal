...
We therefore frame the survey not around the values of individual
coefficients --- which are conventional labels for combinations of
underlying interactions --- but around the regions of coupling space,
and the combinations of invariants those regions encode, that
activate the Gertsenshtein amplification.  We use
PolyChord~\cite{Handley:2015fda,Handley:2015vkr,Skilling:2004pqw,Ashton:2022grj}
as the nested sampler throughout this work.  The nested-sampling
output supports both scalar diagnostics --- the Bayesian evidence
$\mathcal{Z}$, which records the prior-averaged amplification of the
theory, and the Kullback--Leibler divergence
$D_{\mathrm{KL}}$~\cite{Kullback:1951zyt,Handley:2019mfs}, which
records how concentrated the response is in coupling space --- and
posterior-distribution visualisations of the same chains, produced
with anesthetic~\cite{Handley:2019mfs} as corner plots and as
per-tile panel maps over the cubed-sphere atlas
...
of this exit value over its Einstein--Maxwell
...
\begin{figure}[t]
  ...
  \caption{Gnomonic cubed-sphere atlas used to sample the direction
    ...
    (face~$2\!\uparrow$, transparent blue) is one \emph{cube-tile} of
    the atlas, partitioned into $3\times 3$ sub-tiles whose
    constrained-prior chains run independently; its image on the
    sphere is the matching \emph{sphere-tile}.  Dotted lines from the
    origin through cube-tile anchor points are gnomonic projection
    rays; the small disks mark where each ray meets the sphere.  The
    green arrow is one sample vector $\theta = r\,\hat r$ through
    the central shaded sub-tile: the dark segment inside the cube and
    the arrow segment emerging past the face show the two-step
    decomposition of the radial-direction prior into a magnitude $r$
    and a direction $\hat r$.  The figure idiom
