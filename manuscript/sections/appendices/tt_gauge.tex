...
Beyond the static obstruction above, the gravitational wave itself
sources additional $\FieldH{_\mu_\nu}$-dependence in the Maxwell
stress. At first order around $\bar{F}_{\mu\nu}\neq 0$ the perturbed
stress acquires a piece
$\delta T_{ij}\supset\tfrac{1}{2}\bar{B}^{2}h_{ij}$ --- a
self-coupling with no $\delta A$ involvement. The wave equation
\cref{TTGLorenzWaveEq} for $\bar{h}_{ij}$ therefore carries an
$h$-dependent source: the GW sources itself through its interaction
with the background. Hwang \&
Noh~\cite{hwangnoh2023graviton} show that the conventional TT-gauge
result survives this back-reaction only in the limit of an exact
plane wave on a uniform constant background.
...
$\Box^{2}\xi^{\mu}=0$; the four free components of $\xi^{\mu}$ in
each Fourier mode are exactly the right count to remove four
$\bar{h}_{\mu\nu}$ combinations and leave the two TT polarisations.
A curvature-squared term in the action --- $R^{2}$ in Riemannian
gravity, or $\MAGFt{}^{2}$ in the Poincar\'{e} theories of
\cref{PGTEnumeration} --- gives $\bar{h}_{\mu\nu}$ a
higher-derivative equation of motion while $\xi^{\mu}$ stays
second-order; the new equation propagates an additional scalar mode
that the residual gauge cannot reach. In $f(R)$ gravity this is the
scalaron; in PGT, the analogous propagating combination of the
metric trace $h^{\sigma}{}_{\sigma}$ with the torsion-trace vector.
