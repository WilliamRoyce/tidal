...
% TODO: \paragraph*{Primary constraints and singular kinetic matrix}
% \label{DiracBergmann}
%
% A second obstruction to the Legendre transform of \cref{LegendreTransform}
% arises when the kinetic matrix K_{ij} is singular from the start --- gauge
% potentials (A_\mu), lapse/shift in linearised gravity, or any field that
% enters the action algebraically. This is distinct from the LPS/JLM
% constraint-promotion case above, which concerns theories whose K is
% invertible at \varepsilon=0 but which develop new degrees of freedom when
% \varepsilon is turned on.
%
% Content to write (~200 w):
%   1. K singular => primary constraints (cite henneaux1992quantization).
%      Naive \pi = K v not solvable; Legendre transform not invertible.
%   2. Correct treatment: Dirac-Bergmann procedure --- identify and classify
%      all first- and second-class constraints, project onto constraint surface,
%      replace Poisson brackets with Dirac brackets (cite henneaux1992quantization;
%      also dirac1950generalized if in bib, else Henneaux alone suffices).
%   3. TIDAL's choice: retain velocity-form \ham = \sum \pi v - \lag^{(2)} of
%      \cref{LegendreTransform} without performing the Dirac-Bergmann reduction
%      (computationally prohibitive for the full PGT spectrum).
%   4. Consequence: exported \ham is gauge-non-invariant; energy drift from
%      'tidal measure --what conservation' includes gauge-artifact contribution
%      for all gauge-field theories; drift values cited in \cref{Validation}
%      for such theories are artifacts, not dynamical errors.
%   5. Observable unaffected: conversion probability \cref{GertsenshteinFormula}
%      is gauge-invariant; survey results of \cref{Results} are unaffected.
