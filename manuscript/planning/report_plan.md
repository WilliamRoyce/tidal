...
**v3 architecture cross-reference (added 2026-05-11)**: The methodology section (§3 Computational approach) is now driven by the **v3 architecture pivot** (post-2026-05-08 supervisor meeting). The architecture is settled and durably documented at [docs/V3_ARCHITECTURE.md](../../docs/V3_ARCHITECTURE.md). The Phase 2 manuscript update sequence is captured in detail at [docs/V3_PHASE_D_DESIGN.md](../../docs/V3_PHASE_D_DESIGN.md) — including the v2-vs-v3 comparison table template, the geometry-decision rule (Phase B plane-wave vs Phase E localised headline), and the chain-pull workflow that every campaign result must pass through before becoming a manuscript number. **The §4 Results section is gated on Phase B (HPC campaign re-runs) convergence**: do not start writing results until the relevant chains in [docs/V3_PHASE_TRACKER.md](../../docs/V3_PHASE_TRACKER.md) Phase B table show ✅ done. The methodology section text (§3) *can* be drafted now with placeholder numbers — architecture is settled, only the chain results are pending.
