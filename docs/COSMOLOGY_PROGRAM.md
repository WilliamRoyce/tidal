...
| **#582 — legacy's contortion identity is wrong** (not a contortion, and the opposite sign to xTensor's `ChristoffelCDCDT`); every `R̃` torsion term it produced differs, so torsion **mass** terms are affected, not just a total derivative | a decision on scope: re-derive the three affected legacy theories and diff, and judge whether any thesis-era torsion number depended on them (#222's wrong-sign-mass puzzle is a candidate origin) | **raised with the user 2026-09-16**; correction owned at M3 | #582 (priority high); `perturbation_tooling.md` Part 4 / §5 |
