...
`literature/2003.02690/` for the tracking/frozen solutions. *(Amended 2026-09-18: under
the provisional zero-background-torsion decision (user, 2026-09-17; row *Background torsion is exactly zero*), tracking torsion is **excluded for now** — the check asks the sharper question
"does the theory admit exactly zero torsion on the ΛCDM background?", and a theory whose
equations drive a background torsion is out of scope rather than handled approximately.)* The check is a
...
| **Rung order** | `O0 → O1 → O2 → O4a → O3 → O4b/V-modes`. Technically neutral, so a pure scientific-priority call. **Gated on #503**: O4a is only cheap if the operator is `n = 0` and `β` is constant over recombination. *Dated note, 2026-09-13: **O1 is parked** (#498, milestone Parked — replaced by the self-validation ladder, row below) and **O4a is conditional on what #503 finds at M3**; the order is kept, not rewritten, until that answer exists* *Dated note, 2026-09-18: under the provisional zero-background-torsion decision (user, 2026-09-17; row *Background torsion is exactly zero*), **O4a** — isotropic birefringence from a homogeneous torsion mode, which needs `T̄ ≠ 0` — is **no longer a planned rung**. The order is kept as written until the supervisor confirms the decision; O4a then moves to Parked with the decision's reopening condition* | user, 2026-09-04 (H2 recommended); note 2026-09-13 | §Rung order, `observable_ladder.md` §5 |
...
| **Config layer** | Build it provisionally, **mark it disposable** — WS1's real surface must *replace* it, not extend it. The failure mode is a provisional layer becoming the de facto config by accretion *Superseded for the theory loader, 2026-09-15 (user, on R-1 §3.8): D-B fixed the format, so the loader is **permanent and schema-versioned** — row *Theory loader is permanent*. The rule still binds any config not yet designed* | orchestrator, 2026-09-04 | #527 |
...
| **Admissible theories** | Scoped **explicitly**: only theories admitting the assumed background are in scope, the user judges whether theirs qualifies, and the background-EOM residual is a **first-class per-theory gate** rather than a diagnostic *Sharpened 2026-09-18 under the provisional zero-background-torsion decision (user, 2026-09-17; row *Background torsion is exactly zero*): admissible means the theory admits **exactly zero** background torsion on the ΛCDM background; the tracking class of `2003.02690` is excluded for now* | user, 2026-09-05 | `spectator_route.md` §3, #501, #531 |
...
| **The certified PSALTer configuration, and what it is independent of** | Wolfram **14.3.0** × xAct **1.3.0** bundle × PSALTer v2.0.2 `bb45adb0` × local registration of `LinearlyIndependent` (the engine-bundled/userbase `ResourceFunctionHelpers` file, sha256 `7bc228a2…`) and `PolynomialDegree` (the committed definition notebook, sha256 `c233e226…`) under **fixed UUIDs** `d40a8dd6-…`, `2f89f2e6-…`. `verify --require-psalter` asserts *provenance* — those UUIDs on master and subkernel, the engine under its mount, the xAct fingerprint — and each assertion was watched fail. `scripts/psalter/ensure_registered.sh` re-creates the registration idempotently (rebuild simulated from an empty registry: identical UUIDs, exit 0). **Independent of the Wolfram-ID cloud login** (asserted logged in and logged out, stored credentials untouched) and of the engine minor version (14.2.1 cross-check MATCH, `evidence/tier1-20260911-engine-142/`, then retired). Nobody's login is logged out: it lives in the user's own home mounts, not the repo **Extended 2026-09-17 (D-C = O1′):** the certified configuration now also carries **xPand 0.4.4**, installed by `scripts/install-xpand.sh` and checked by `verify-wolfram-setup.sh` as its own item — **DEGRADED (exit 2)** when absent or at another version, watched failing in both modes. It is deliberately **not** part of `EXPECTED_XACT_FINGERPRINT`, which stays the four-package xAct bundle that this row and several documents quote. The xMAG chain (five directories) is a **research** install and is not checked | orchestrator, 2026-09-11 | `docs/cosmology/evidence/tier1-20260911-pass/`, `scripts/psalter/README.md`, #559 |
...
| **Convention carriage is a pipeline rule, not a user input** | PSALTer's and CAMB's Minkowski signatures are each a **fixed constant**, different from each other, both determined from source by R-1. The user writes the Lagrangian in **one documented convention** (R-1 recommends which; the sign-sensitive spectrum branch makes PSALTer's the natural candidate); the pipeline maps deterministically into each backend's; every derived artifact records the convention it is in (`conventions.signature`, `epsilon0123`) and every consumer asserts it. A slip changes a ghost verdict (`spectrum_design.md:204-207`), so R-1 designs the check *Resolved 2026-09-15 (R-1): the premise that the two backends' signatures differ was **wrong** — CAMB declares `(+,−,−,−)` like PSALTer (#569); row *Conventions*.* | user, 2026-09-13 (rounds 5–6) | R-1 (#566), `handoffs/I-S1A.md` |
| **Validity mechanism follows group practice** | Where a per-sample veto is raised in Cobaya, hard or soft, and how a rejected point is reported is **inherited, not invented**: I-532's memo establishes it from TorC's and the polology codes' *code* first, and **v1 matches that practice**. The flag schema supports both severities so the group's choice is representable (one evaluator, never two code paths — #454); `tidalcosmo/validity/README.md:28-30` is amended to whatever that practice implies; M1b implements *Evidence 2026-09-15 (R-1 §3.5, #573): the group applied health conditions **offline** — TorC fixed the couplings and put only numerical-validity exclusions into Cobaya external priors; polology treats health as an offline prior on the couplings, then reweights — so there is **no group precedent for a per-sample spectrum check inside Cobaya**, and an external prior sees only parameter values. I-532's memo starts from these citations and confirms them from the codes. **User, 2026-09-15: if the codes confirm health is only ever screened offline, v1 has no per-sample veto — a legitimate outcome, not a gap**; the R-1 startup gate still guarantees the matching derived spectrum exists* | user, 2026-09-13 (round 6) | #532, `docs/cosmology/veto_placement.md` (I-532) |
...
| **D-A — Stage 1 is one committed Wolfram package run by a fixed driver** | Python writes the theory as a **WXF data file** (kernel-free `wolframclient` serializer) and launches **one `wolframscript` subprocess per derivation** running a **fixed `driver.wls`**; Python never writes Wolfram code. User text is parsed held and whitelisted before PSALTer runs; `Catch` + `Exit[1]` (#561) lives once in the driver; the verdict is the artifact, never the exit status. Precedent: every mature Wolfram physics tool reads a model with one package (FeynRules, SARAH, PSALTer's own materials); FlexibleSUSY runs one process per derivation. **Measured**: the package reproduced `vector_smoke.wls`'s wave operator `SameQ`; eleven malformed and injected inputs refused. **Rejected, measured**: a `wolframclient` session — `DefField` never returns inside one, the timeout is ignored, `Quit[]` silently restarts a kernel without PSALTer; Python-templated scripts (legacy). **Re-run independently by the orchestrator from the committed bytes** before recording | user, 2026-09-15, from R-1 | `interfaces_decision.md` §2, #566, #572 |
| **D-B — Option A′: a theory YAML included into Cobaya run files** | The user writes a **theory file** — `fields` (`indices`, `print_as`, `print_source_as`, optional `symmetry`), `lagrangian` as a **mapping coupling → operator in PSALTer notation** (linear in the couplings by structure; a product of couplings is its own coupling), `derivation` — and includes it in standard Cobaya run files with Cobaya's own **`!defaults`** (or inline under `model:`); priors only in top-level `params:`. **Two commands**: `tidalcosmo derive run.yaml` (needs Wolfram) then `cobaya-run run.yaml` (never can), mirroring `cobaya-install` → `cobaya-run`. The derived spectrum lives in a **content-addressed data store** (`<store>/v<schema>/<fp[:2]>/<fp>/{spectrum.wxf, manifest.json}`; `spectra_path:` → `TIDALCOSMO_SPECTRA_PATH` → `$XDG_DATA_HOME/tidalcosmo/spectra`), keyed by a fingerprint the user never handles; the Theory registers couplings in `get_modified_defaults`, reports the fingerprint as its version (resume refuses a different spectrum) and **refuses at construction with `ComponentNotInstalledError`** when the spectrum is missing, stale, corrupt or in other conventions (twelve cases run). M3's re-derived specs go to a committed store in the repository, never `examples/data/`. Rejected: a Wolfram model file (GUM's precedent needs a model library PSALTer lacks); a fixed operator vocabulary (hi_class/EFTCAMB — no arbitrary Lagrangian) | user, 2026-09-15, from R-1 | `interfaces_decision.md` §3 |
| **Conventions — each quantity takes the convention of the tool that defines it** | Signature **`(+,−,−,−)`** and **`ε₀₁₂₃ = +1`** (PSALTer; CAMB declares the same signature and defines no `ε`) for **both** branches; **CAMB's perturbation-variable definitions** (`camb.symbolic`) at the seam; Cobaya's parameter names. Nothing converted at run time (#513); a formula imported from another convention is rewritten once, where imported, with a test. Every manifest records `conventions` and every consumer refuses others; a reference theory with a published verdict runs in CI; a `camb.symbolic` variable-mapping test at the seam (I-532). Legacy and Ma–Bertschinger comparisons attribute every difference to convention or physics (#570). Open: `β` sign and polarization handedness for O4 (#571). `spectrum_design.md` §4.3 corrected (#569) | user, 2026-09-15, from R-1 | **`docs/cosmology/conventions.md` (canonical)** |
| **Theory loader is permanent and schema-versioned** | I-S1A-core builds the real Option A′ loader, not a stand-in: a `schema` version, an optional per-field `symmetry` key validated against PSALTer's 14 classes, and a fingerprint whose canonical form **drops defaulted and absent keys** (the prototype merged defaults into the hash, so any new default would re-derive every stored spectrum). Unknown keys stay refused, so a theory needing a missing piece is refused, never mis-derived. An absent-by-default key keeps old fingerprints only when absence reproduces the old derivation exactly; otherwise `schema` is bumped. **Named gaps, each additive**: expansion geometry and derived fields (`F = dA`) — I-S1B; gauge-symmetry declarations — I-S1B, provisional; the solver's background/linearization/gauge block — M3, with its own fingerprint outside the spectrum's. No `EXPIRES-WITH: #490`; M1b builds on the loader rather than replacing it | user, 2026-09-15, on R-1 §3.8 | `interfaces_decision.md` §3.8, `handoffs/I-S1A.md` |
| **`wolframclient` is core at M1b, an extra until then** | Sampling needs only `wolframclient.deserializers` and `derive` only `wolframclient.serializers`; importing those loads no `wolframclient.evaluation`, `zmq` or `pkg_resources` (R-1 §2.2, re-checked read-only). The `pkg_resources` deprecation does not reach them: only plugin discovery (`serializers/encoder.py:9, 39`) touches it, through a lazy wrapper that returns empty when it is missing (`utils/api.py:152-154`), and export worked in a venv without it; upstream's fix (#41) is unreleased. **Cost accepted:** 1.4.0 declares numpy, requests, oauthlib and the time-zone, async-HTTP, ZeroMQ and CA-certificate packages, installed by core but never imported on the sampling path — one official library rather than a hand-written WXF parser. **So:** no `pyproject.toml` change in Wave 1 (I-532 stays its sole owner); I-S1A-core imports the two modules **lazily, inside functions**, under the `wolfram` extra; **M1b promotes it to core** with `cobaya`, pinned `<2`, with a test that the sampling path never loads `wolframclient.evaluation`, `zmq` or `pkg_resources` | user, 2026-09-15 | `handoffs/I-S1A.md`, `handoffs/M1b.md` |
| **D-C — FRW derivation tooling: O1′** | The FRW branch is built on **xPand 0.4.4** (3+1 split, SVT, both CAMB-named gauges, second order, run natively in `(+,−,−,−)` with `n·n = +1`), the **post-Riemannian rewrite in pure xTensor** (`ChangeCurvature[L, CDT, CD]`, then the connection-difference tensor substituted by the contortion of the torsion — **the corrected identity, never legacy's**, #582), the lane's **SVT rule set** for rank-3 torsion (24 = 8 + 6·2 + 2·2, imported from Aoki et al. with the family-A sign map of `conventions.md` §2.1) and its **η-space Euler–Lagrange operator**; the exports (solver forms, eikonal, line-of-sight sources) are **build-own** — no tool provides them. ≈ 6 engineer-days of M3 tooling. **Why not the alternatives:** O3 (xMAG for the connection algebra) buys nothing while the geometry is metric-compatible — rewriting `Γ = LC + K(T)` and varying in `(g, T)` **is** the Palatini variation of a Riemann–Cartan connection, since `δΓ` metric-compatible ⟺ `δK` ⟺ `δT` — and it would add five GPL-2+ install requirements and one-package-per-kernel discipline; it is adopted the day **non-metricity** (`∇g ≠ 0`) or a variation with respect to `Γ` as a symbol enters. O2 (xPert only) reimplements xPand for 10–15 d and is the fallback if upstream ever breaks. O4 (xMAG's induced decomposition) runs but is untested as an FRW engine. O5 (port legacy's decomposer) is rejected on design **and** correctness (#582). O6 (fork xPand) is unnecessary — torsion enters through rules xPand accepts by design, so there is no shim and no patch — and a fork would drift (the `slegner/CAMB` precedent, #498) and put the derivative under GPL-3 while this repository is MIT. **Consequences:** (i) the rewrite is **one committed function shared by both branches**; (ii) **xPand becomes an install requirement** — promote `install_xpand.sh` to `scripts/install-xpand.sh`, add its `$Version` to `verify-wolfram-setup.sh`'s fingerprint, a `WOLFRAM_GUIDE.md` step, re-certification (its own change before I-532; #580); (iii) **xMAG stays installed as a required oracle**, not an optional one: M3 gates the rewrite against xMAG's own `ContorsionCDT` relation **and** checks that varying with respect to `Γ` agrees with rewrite-then-vary — the equivalence R-C did not run | user, 2026-09-17, from R-C (#567) | `perturbation_tooling.md` Part 4; `conventions.md` §2.1 |
| **Background torsion is exactly zero (provisional)** | We expand about a background with **exactly zero torsion**: the new fields and couplings change only the perturbations, on the unchanged ΛCDM background, which is what "spectator" means here. **Removes:** any calculation with nonzero background torsion — isotropic birefringence from a homogeneous torsion mode (O4a / O4-iso, D5), the tracking class of `2003.02690`, and R-C's round-4 planning allowance for "a small nonvanishing background torsion below the tolerance" (`rc_planning_record.md:119-121`, archived and not edited). **Keeps:** every perturbation-level channel, including parity mixing and Gertsenshtein-type conversion, and the background-consistency check (#501), which now asks whether the theory admits exactly zero torsion on ΛCDM. **The tension:** from `2003.02690`, zero torsion is exact only in some classes (`σ₃ = 0`, or Einstein–Cartan), which is the open FRW question (#531). **Reopening condition:** a theory of interest does not admit zero background torsion on FRW — then reopen #591 and this row. **Provisional:** on the agenda of the supervisor meeting of 2026-09-18 (`docs/meetings/2026-09-18_supervisor.md` §1.1, two questions); confirmed or revised there. Not the reason for the decision, but consistent with it: xPand cannot at present take a nonzero background for a torsion-shaped field (#591, diagnosed) | user, 2026-09-17 (provisional) | `perturbation_tooling.md` §1.2, §5 L11; #591 (closed with its reopening condition) |
...
| 1 | R-1 — the interfaces: user input → derivation → artifact → Cobaya (decides D-A, D-B, convention carriage) | #566 | **yes** (≤ 30 min; 23 min 52 s) | `docs/cosmology/{interfaces_decision,conventions,r1_planning_record}.md`, `scripts/research/interfaces/` | `cosmo/r1-interfaces` (#568) | **merged** ✅ 2026-09-15. CI 34963909180 + 34967874754 success on the PR head. **Lane evidence re-run by the orchestrator from the committed bytes** (534 s wall): package `.mx` and `spectrum.wxf` `SameQ` to `vector_smoke.wls` (digest `36115df6…`), perturbed control `proved-different`; 11/11 bad inputs refused, nothing stored; 12/12 Cobaya gate cases as reported; static check passes on the Cobaya-facing modules and fails on the three launchers; session route reproduced (timeout ignored, `Quit[]` raises, `DefField` hangs 120 s); verify exit 0 before and after. The criterion "three routes agree" was met for two: the session route cannot derive, which is itself the finding. Decided with the user: D-A, D-B (A′), conventions, a permanent loader (§3.8). Issues #569–#573 |
| 1 | R-C — cosmological perturbations: requirements, methods, tools (decides D-C) | #567 | **yes** (≤ 2 days) | `docs/cosmology/perturbation_tooling.md`, `scripts/research/perturbations/`, additive `Applications/xAct/xPand/` | `cosmo/rc-perturbation-tooling` (#574) + closing pass `cosmo/rc-closing-pass` (#588) | **merged** ✅ 2026-09-17 (both). CI 35210615226 success on the PR head. Recommends **O1′** for D-C: xPand 0.4.4 as the metric engine (loads unmodified, native `n·n = +1`, both CAMB gauges), the **pure-xTensor** post-Riemannian rewrite, the lane's SVT rules and η-space Euler–Lagrange operator; build-own the exports; ≈ 6 d of M3 tooling. **Re-verified by the orchestrator from the committed bytes**: verify exit 0 before and after, the author's xPand example `identical`, A2 mixed-index `identical`, the sign globals `{1,1,1,1,1}` for xTensor/xPand/PSALTer and `$RiemannSign = −1` after xMAG loads, the six approved directories and layer 1 unchanged. Follow-up lane corrected its own xMAG verdict — **xMAG works as documented; the sign was its undocumented load-time global** (#585) — leaving Part 4 and O1′ unchanged. Legacy's contortion identity is wrong (#582, scope decided programme-only). **Closing pass (#588, CI 35221106302 success):** the slice ε is PSALTer's own and is *stated*, not derivable (#590, now in `conventions.md`); the family-A import map was **run** in both contortion families (counts and both second-order actions identical, a parity-odd Lagrangian flips cleanly); the lost simplification is **xBrauer's**, measured — it reproduces alone, xMAG adds nothing, `Master` is the discriminator, two rival explanations excluded; a **new xPand defect in our signature** (`SetSlicing` hard-codes the slice determinant sign, so the slice ε is wrong in our signature — parity-odd sector only, #589); and a silent truncation in its own probe, amended in `CLAUDE.md` at the rule's site. **Orchestrator re-ran f7 and f8 from the committed bytes**: the xBrauer attribution and all five of its verdict sentinels reproduce, and `XPAND_1754_DETERMINANT_HARDCODE_BITES=True` only in our signature, not in mostly-plus. Issues #575–#590 |
| 1 | R-C follow-up — #591 diagnosed; background torsion out of scope; guards against our own silent mistakes | #591 | **yes** | `docs/cosmology/{perturbation_tooling,rc_planning_record}.md`, `docs/meetings/2026-09-18_supervisor.md`, `scripts/research/perturbations/`, `tests/test_research_*` | `cosmo/591-background-scope` (#592) | **merged** ✅ 2026-09-18. CI 35352106421 success on the PR head. #591 is real but narrow: xPand fails on a background that is a sum of terms along the normal (the torsion shape); zero and one-term backgrounds work. **Two of its own claims retracted**: the "silently empty first order" was a helper of ours called with four arguments, and a plan-only withdrawal of the xBrauer explanation was wrong (f10 confirms it). **Re-verified by the orchestrator from the committed bytes**: f9–f11 and verify, and the new pre-kernel lint watched refusing — every defect fixture exits 1, and `run_lane.sh` exits 4 without starting a kernel. Your zero-background-torsion decision is recorded as provisional (register) |
| 1 | I-532 — CAMB seam, background protocol, flag schema, veto placement | #532 | — | `tidalcosmo/{background,spectator,validity}/`, `tests_cosmo/data/camb_oracles/`, `docs/cosmology/veto_placement.md`, `pyproject.toml` (camb line only) | `cosmo/i532-camb-seam` | **dispatched** 2026-09-18 — no lane; two questions answered (design-doc amendments at site; the oracle-drift rule) |
| 1 | I-S1A-core — permanent theory loader, spectra store, driver launcher, WXF reader + Stage-2 dataclasses | #527 | — | `tidalcosmo/{config,derive,spectrum}/` (Python only) | `cosmo/is1a-stage1-python` | held — I-532's flag type; D-A, D-B, carriage rule and loader permanence recorded |
| 2 | I-S1B — Stage-1 Wolfram package + driver, exporter, `tidalcosmo derive` CLI, cost run, `A[0]` | #495 | **yes** | `tidalcosmo/derive/wolfram/` | `cosmo/is1b-stage1-wolfram` | held (outline) — D-C, I-S1A-core |
| 2 | M1b — Cobaya Theory on the permanent loader, veto wiring, ΛCDM identity, #515 | #494 #515 | — | `tidalcosmo/spectator/theory.py`, `tidalcosmo/__init__.py` | `cosmo/m1b-cobaya-theory` | held (outline) — I-532, I-S1A-core |
...
9. **Wave 1 orchestrator commitments** — copied from the six prompts at dispatch
   (2026-09-13), per item 6. *R-1 (#566):* ✅ **done 2026-09-15** — options put to the user (D-A, D-B, conventions, and
   loader permanence after a §3.8 follow-up); four register rows; R-C, I-532, I-S1A-core, I-S1B
   ...
   the session route, the perturbed control, all failure fixtures and gate cases. *R-C (#567):* ✅ **done 2026-09-17** — `verify --require-psalter` exit 0
   before and after, re-run from the committed bytes together with `probe_load`, `a2`, `f3`,
   `f5` and `f6` (the author's xPand example `identical`, A2 mixed-index `identical`, sign
   globals `{1,1,1,1,1}` for xTensor/xPand/PSALTer and `$RiemannSign = −1` after xMAG loads);
   R-C's own `before.layer1` and `after.layer1` manifests are byte-identical (942 files,
   `9e2c81a4…`) and an independent recompute matches them line for line; exactly the six
   approved directories exist, 64 files, each stamped. **Still open:** copy R-C's 31 fetched
   `literature/` directories into the main checkout and regenerate `literature/README.md`
   there; **record D-C once the user chooses**, then update I-S1B and the M3 outline and mark
   I-532 READY; request the Wayback snapshot of the xPand tarball from an unrestricted machine
   (this container's TLS handshake to archive.org is refused, and the orchestrator shares it). *I-532 (#532):*
   ...
   register. *I-S1A-core (#527):* pre-dispatch — `wolframclient` placement **decided 2026-09-15**
   (row *`wolframclient` is core at M1b*): no `pyproject.toml` change this wave, so nothing to
   apply; confirm the prompt still says lazy imports of the serializer/deserializer modules under
   the `wolfram` extra; no `platformdirs` (the store uses the XDG variables directly); at merge run the private-globals guard test on the lane
   ...
   *Boundary:* `validity/README.md:28-30` amended (`derive/README.md:12` done
   2026-09-15); `git grep EXPIRES-WITH` shows the PSALTer pin and **no `#490`** (the loader is
   permanent); the carried-forward register current; one
   bump, tagged.
...
> **State, 2026-09-18. Wave 1: R-1, R-C and its follow-ups MERGED, D-C recorded (O1′); I-532 DISPATCHED; zero background torsion provisional, pending the supervisor. Wave 0, its completion wave and the hardening pass are COMPLETE, and
...
> **`R-1` (#566) merged 2026-09-15** and its decisions are the four register rows dated that day
> (D-A, D-B, Conventions, Theory loader is permanent). **`R-C` (#567) is DISPATCHED (2026-09-15) and
> holds the Wolfram lane.** Held behind it, in order: `I-532` (its R-1 inputs recorded; waits for R-C
> under the one-at-a-time cadence) → `I-S1A-core` (on I-532's flag type). Held for Wave 2 as outlines: `I-S1B`, `M1b`. The
...
| **R-C's verdict (D-C)** | ✅ **decided 2026-09-17: O1′** | — | register row *D-C — FRW derivation tooling* |
| **xPand's `$CovDs` defect and its five-line caller-side wrapper** (#583) | M3's package; the upstream report is drafted but not filed | M3 | `perturbation_tooling.md` §3.2; `scripts/research/perturbations/xpand_upstream_issue.md` |
| **xMAG as a REQUIRED oracle of the rewrite** (#585; D-C consequence iii) — it works as documented once its load-time `$RiemannSign = −1` is reset; the gate checks our substitution against xMAG's own `ContorsionCDT` relation **and** that varying with respect to `Γ` agrees with rewrite-then-vary (the equivalence R-C did not run) | M3's rewrite existing | M3 (0.25 d) | `perturbation_tooling.md` §3.3, Part 4 §4.2; `xmag_upstream_issue.md` |
| **xPand's slice ε in our signature is wrong** (#589): `SetSlicing` hard-codes the induced metric's determinant sign, so in our signature the slice ε squares to `+6` where the four-index ε contracted with the normal gives `−6`. Parity-odd sector only | M3 writes the parity-odd sector in the **four-index** form; the upstream report is drafted but not filed | M3 | #589; `perturbation_tooling.md` §7; `conventions.md` §2 |
| **xPand and a nonzero background value** (#591, **closed** with its reopening condition) — xPand fails when a field's background is a sum of terms along the normal, the shape a torsion background takes; zero and one-term backgrounds work, and a failed attempt does not affect later calculations. *(Corrected 2026-09-18: the "sometimes silently empties the first order" this row used to carry was a helper of ours called with four arguments instead of three — never xPand's — and is retracted.)* D5, O4-iso and a gate with `T̄ ≠ 0` are **out of scope** under the provisional decision, not blocked | nothing while the decision holds; if it is reopened, a working route for nonzero backgrounds (an xPand fix — `upstream/xpand_email.txt` item 3 is drafted — or our own) | only on reopening | #591; row *Background torsion is exactly zero* |
| **After the supervisor meeting (2026-09-18)** — if the zero-background-torsion decision is confirmed: move O4a from the rung order to Parked with the reopening condition, and re-milestone #558 (`β` over recombination) and #579 (O4-iso homogeneous mode) to Parked; if revised: reopen #591 and plan the nonzero-background route | the supervisor's answer to agenda §1.1's two questions | at the meeting | row *Background torsion is exactly zero*; `docs/meetings/2026-09-18_supervisor.md` |
| **Guards for any xAct work, required in M3's prompt and in I-S1B** — `wl_lint.py` before every kernel (already run by `run_lane.sh`), `RCOrder` rather than bare `ExtractOrder`, `PACKAGE_FACTS.md` read first, and `upstream/check_snippets.py` plus a claims table for anything sent upstream. Rationale: most of R-C's own mistakes were silent Mathematica failures (a mis-called function returned unevaluated, then turned into a plausible answer), not package defects | M3's and I-S1B's prompts being written | at their rewrite | `scripts/research/perturbations/{PACKAGE_FACTS.md,README.md,wl_lint.py,upstream/}`; `perturbation_tooling.md` §8 |
| **Upstream — the user's to send** (updated 2026-09-18 from the follow-up's last commit): **filed or posted** — [xMAG #2](https://github.com/THelpin/xMAG/issues/2) and its correction comment (three errors of ours fixed), [xBrauer_Bundle #2](https://github.com/THelpin/xBrauer_Bundle/issues/2), [xBrauer_Bundle #3](https://github.com/THelpin/xBrauer_Bundle/issues/3) (the contraction regressions). **Still to send:** a one-line comment on xMAG #2 linking xBrauer_Bundle #3, and the xPand email with its four scripts. Every claim is checked in `upstream/claims.md` | the user | when convenient | `scripts/research/perturbations/upstream/` |
| **`literature/README.md` is stale** — 30 of R-C's fetched papers are on disk but not in the generated inventory | arXiv to stop answering this host with HTTP 406 (throttling, measured eight times; not a header or batch-size bug), or a run from another host | whenever it answers | #587; the note in `literature/README.md` |
| **Wayback snapshot of the xPand tarball** | a machine whose TLS handshake archive.org accepts (this container's is refused) — **the user's to fetch** | before the install route is relied on by others | `scripts/install-xpand.sh --wayback`; memo §6 |
| **xPand as an install requirement** | ✅ **done 2026-09-17** — `scripts/install-xpand.sh` (a matching install is now a no-op, a different one is refused), its own `verify-wolfram-setup.sh` check (**DEGRADED, exit 2**, when absent or at another version — **watched failing in both modes**, then restored byte-exactly against R-C's manifest and re-certified exit 0), a seven-step `WOLFRAM_GUIDE.md`, and rows in `README.md` and `scripts/README.md` | — | `docs/cosmology/evidence/xpand-promotion-20260917/` |
| **#582 — legacy's contortion identity is wrong** (not a contortion, and the opposite sign to xTensor's `ChristoffelCDCDT`); every `R̃` torsion term it produced differs, so torsion **mass** terms are affected, not just a total derivative | **Scope decided by the user, 2026-09-16: programme-only.** M3 ports the *corrected* identity (never `stage1_engineering_plan.md:519-525`'s text), and `tests_cosmo/data/oracles/README.md` gains a drift class saying those frozen specs carry legacy's wrong `R̃` terms so M3 attributes the difference. **Legacy is not re-derived and no thesis-era number is re-checked** — that question stays open and unanswered, by decision, not by oversight | M3 | #582 (priority high); `perturbation_tooling.md` §5; #222 is a candidate origin, unverified |
| **Wolfram-side `Exit[1]` on abort** (#561's deferred half) + PSALTer's bare `Quit[]` (#572) | D-A recorded: `Catch` + `Exit[1]` in the fixed `driver.wls` | I-S1B | #561, #572, `handoffs/I-S1B.md` |
| **Theory-schema gaps** (all additive, R-1 §3.8): expansion geometry (which formulation supplies R and T from which fields), derived fields (`F = dA`), gauge-symmetry declarations; the operator whitelist beyond the Vector theory | a torsion theory through the package | I-S1B | `interfaces_decision.md` §3.8; register row *Theory loader is permanent* |
| **Solver-only theory block** — background fields, field → background + perturbation map, gauge choice; its own fingerprint | D-C; #500 | M3 | §3.8; `spectrum_design.md:351, 368-371` |
| **Convention drift class** for the legacy-oracle mapping (#570); **O4 `β` sign and polarization handedness** (#571) | M3's mapping; O4 design | M3; M5 | `conventions.md` §6–§7 |
| **Per-sample spectrum health check — no group precedent** (#573) | I-532's veto memo | I-532 | register row *Validity mechanism follows group practice* (evidence note) |
