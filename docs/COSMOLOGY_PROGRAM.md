...
| **xPand's `$CovDs` defect and its five-line caller-side wrapper** (#583) | M3's package; the upstream report is drafted, unfiled | M3 | `perturbation_tooling.md` §3.2; `scripts/research/perturbations/xpand_upstream_issue.md` |
...
| **xPand's slice tensor `epsilonh` relative to `ε` and the normal — not checked** | a kernel probe before the parity-odd torsion rules are used | before O4-aniso work | `perturbation_tooling.md` §1.2, §7.3 |
