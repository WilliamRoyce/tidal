...
| **The family-A import map with a non-vanishing background torsion — never exercised** | supplying a background rule alongside the perturbation rule made every order-1 piece vanish, so the half-applied map was not tested | with #501's `T̄ ≠ 0` work (M3) | `perturbation_tooling.md` open items |
