...
| **xPand as an install requirement** (promote `install_xpand.sh` to `scripts/install-xpand.sh`, add xPand's `$Version` to `verify-wolfram-setup.sh`'s fingerprint, a `WOLFRAM_GUIDE.md` step, re-certification) | ✅ **approved by the user 2026-09-17**; needs the lane and a re-certification run | **its own change, before I-532 is dispatched** | memo Part 4 §4.3 consequence (ii); #580 |
