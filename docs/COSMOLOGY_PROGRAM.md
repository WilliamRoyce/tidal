...
- **WS6 — Numerical polology**: **design settled by H6** — `docs/cosmology/spectrum_design.md`
  ...
  correct and sufficient (spectrum screens in vacuo; TorC used the same split). Stage-1
  Wolfram engineering split out as handoff **H8** (`docs/cosmology/handoffs/H8.md`, ready to dispatch).
...
| H8 | **Stage-1 Wolfram engineering** (PSALTer install, spectrum derivation branch, our exporter, `Method→"Hard"` cost) — **prompt written, ready to dispatch**. The program's first *engineering* handoff. Its `Method→"Hard"` measurement on PGT+EM is a **go/no-go input for the two-stage architecture** | `docs/cosmology/handoffs/H8.md` |
...
| **H8** | **PROPOSED by H6 (user decides whether to dispatch; prompt not yet written):** Stage-1 Wolfram engineering — install PSALTer; the spectrum derivation branch (vacuum quadratic Lagrangian, Barker-native conventions) + auto-generated model script; **our own exporter** of the Stage-1→Stage-2 contract (labeled blocks as coupling-linear coefficient tensors, gauge ranks, massive/massless partition, constraint matrix); measure `Method→"Hard"` cost on PGT+EM. Coordinated with M3's `derive/` port. Scope: `spectrum_design.md` §13 | — |

Dispatch order: **H1 ✅ H2 ✅ H3 ✅ H4 ✅ H5 ✅ H6 ✅ H7 ✅ done. H8 proposed (above).** (H3 absorbed the
