...
| **No working way to give xPand a nonzero background torsion** (#591) — the rule-list route usually **fails** (`$Failed`) and only sometimes silently empties the first order; a tooling gap, not physics or a sign convention (both contortion families behave identically). It **blocks** D5 / O4-iso and any tadpole gate with `T̄ ≠ 0`, and it is why the family-A import map was never exercised with a background | a working route to hand xPand a background value for a user-declared perturbed tensor | with #501's `T̄ ≠ 0` work (M3) | #591; `perturbation_tooling.md` §1.2, §7 (corrected at the sites) |
