...
| 1 | R-C — cosmological perturbations: requirements, methods, tools (decides D-C) | #567 | **yes** (≤ 2 days) | `docs/cosmology/perturbation_tooling.md`, `scripts/research/perturbations/`, additive `Applications/xAct/xPand/` | `cosmo/rc-perturbation-tooling` (#574, draft) | **reported** 2026-09-16 — evidence complete; CI 35087547239 + 35093093122 success. Recommends **O1′** (xPand metric engine + pure-xTensor post-Riemannian rewrite + the lane's SVT rules and η-space Euler–Lagrange operator; build-own the exports; ≈ 6 d for M3). xPand 0.4.4 runs unmodified (no shim); torsion representable with hand SVT rules; **xMAG's `ToDistortion` sign disagrees with xTensor and the textbook** (#585, O3 deferred, O4 withdrawn); **legacy's contortion identity is wrong** (#582, priority high). A follow-up lane is adding commits to the same PR — **merge after it reports.** Issues #575–#586 |
...
> **State, 2026-09-16. Wave 1: R-1 MERGED, R-C REPORTED (follow-up lane running). Wave 0, its completion wave and the hardening pass are COMPLETE, and
...
| **R-C's verdict (D-C)** | R-C's follow-up lane to report, then the user's choice between O1′ and a deferred O3 | at R-C's merge | memo Part 4; register row "FRW derivation tooling" (written when the user decides) |
