...
| 1 | R-C — cosmological perturbations: requirements, methods, tools (decides D-C) | #567 | **yes** (≤ 2 days) | `docs/cosmology/perturbation_tooling.md`, `scripts/research/perturbations/`, additive `Applications/xAct/xPand/` | `cosmo/rc-perturbation-tooling` | **ready** — the user dispatches |
...
9. **Wave 1 orchestrator commitments** — copied from the six prompts at dispatch
   ...
   register. *I-S1A-core (#527):* pre-dispatch — D-A uses `wolframclient`'s **kernel-free**
   WXF serializers, today only in the `wolfram` extra: decide whether I-S1A-core's modules
   import it under the extra or it moves to core (it must be core by M1b, whose Theory reads
   `spectrum.wxf` at sampling time), and apply that before dispatch; no `platformdirs` (the
   store uses the XDG variables directly); at merge run the private-globals guard test on the lane
...
> **State, 2026-09-15. Wave 1: R-1 MERGED, R-C READY. Wave 0, its completion wave and the hardening pass are COMPLETE, and
...
> (D-A, D-B, Conventions, Theory loader is permanent). **`R-C` (#567) is READY — the user
> dispatches it.** Held behind it, in order: `I-532` (its R-1 inputs recorded; waits for R-C
