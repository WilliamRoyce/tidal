...
| 1 | I-532 — CAMB seam, background protocol, flag schema, veto placement | #532 | — | `tidalcosmo/{background,spectator,validity}/`, `tests_cosmo/data/camb_oracles/`, `docs/cosmology/veto_placement.md`, `pyproject.toml` (camb line only) | `cosmo/i532-camb-seam` | held — R-C merged (sequential cadence); its R-1 dependencies are recorded |
...
> **State, 2026-09-17. Wave 1: R-1 and R-C MERGED; D-C awaits the user; I-532 next. Wave 0, its completion wave and the hardening pass are COMPLETE, and
...
| **R-C's verdict (D-C)** | **the user's choice** — the memo recommends O1′ and the follow-up left Part 4 unchanged | now (R-C is merged) | memo Part 4 §4.3; register row "FRW derivation tooling" (written when the user decides) |
...
| **xMAG as the oracle test of our post-Riemannian rewrite** (#585) — it works as documented once its load-time `$RiemannSign = −1` is reset; O3 is a later adoption, not a blocked one, and matters only if non-metricity or a Palatini-style connection variation enters | M3's rewrite existing | M3 (0.25 d) | `perturbation_tooling.md` §3.3, Part 4 §4.2; `xmag_upstream_issue.md` |
...
| **xPand as an install requirement** (promote `install_xpand.sh` to `scripts/install-xpand.sh`, add xPand's `$Version` to `verify-wolfram-setup.sh`'s fingerprint, a `WOLFRAM_GUIDE.md` step, re-certification) | D-C recorded as O1′ | immediately after D-C | memo Part 4 §4.3 consequence (ii); #580 |
