...
**CAMB policy:** target **latest upstream CAMB** by default. The `slegner/CAMB` fork is
an *optional hook* used only when the tabulated-background feature is active (TorC-class
analyses); H1 decides inherit-vs-reapply.
...
| **O1** | **Recreate the TorC analysis** with TorC's published `ρ_Λ^eff(a)`, `P_Λ^eff(a)` as input tables (validation case, **not** a foundation) | tabulated-background feature end to end | against published chains (Zenodo 10.5281/zenodo.15866507). **First target.** |
...
| ID | Task | Artifact |
|---|---|---|
| H1 | TorC pipeline audit (paper + slegner/CAMB + slegner/cobaya diffs + Zenodo chains) | `docs/cosmology/torc_pipeline_audit.md` |
...
| H5 | Literature acquisition into `literature/` (writes files) | populated `literature/` + `docs/references.md` |
...
Dispatch order: H1, H2, H5 first (H2 depends on H1's audit). H3 ∥ H4 next. H6 anytime.
...
- **WS5/O1:** published TorC posterior reproduced from Zenodo chains via the general
  tabulated-background feature.
