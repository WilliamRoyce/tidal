...
% Local style overrides — the labels ("IDA", "CVODE", "Modal solver",
% "Explicit scheme?", "Constraint equations?", ...) are all short, so
% the shared `solver` (3.2 x 0.9 cm) and `decision` (1.6 cm) box sizes
% leave a lot of whitespace.  Tighten only for this figure; the global
% styles in tidal-tikz-styles.sty are reused by other diagrams.
\tikzset{
    solver/.append style={
    ...
    decision/.append style={
        minimum width=1.0cm, text width=1.7cm,
...
% --- IDA (dissipation) ---
\node[solver=Orange, right=of d4] (ida3) {IDA};

% --- Default: CVODE ---
\node[solver=Green, below=of d4] (cvode)
    {CVODE};
...
% D4 -> D5 (no)
\draw[arr] (d4) -- node[nolbl, left] {No} (cvode);
