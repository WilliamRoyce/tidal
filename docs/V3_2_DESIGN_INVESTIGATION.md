...
| **B: Log-space reparameterization** | Sphere over `(log\|α_kin\|, α_other, ...)`; `c_i = ±exp(r·θ̂_i)` for log-space dims | ~25 | Sphere stays uniform on log-coords; BUT magnitude `r` rescales exponentially for log-space dims while sign-symmetric dims see `r` linearly — **inconsistent magnitude treatment across couplings**; hidden exponential transform |
