...
| `interfaces_decision.md` | **R-1 (2026-09-15).** How Python drives Wolfram (D-A: one committed package + fixed driver), what the user writes and how the derived spectrum reaches Cobaya (D-B: Option A′ theory YAML, content-addressed store, `ComponentNotInstalledError` gate), convention carriage; §3.8 schema stability. Every claim run or pinned |
| `conventions.md` | **Canonical since 2026-09-15.** Each quantity takes the convention of the tool that defines it: PSALTer's `(+,−,−,−)`, `ε₀₁₂₃ = +1` (CAMB declares the same signature); CAMB's perturbation variables at the seam; PSALTer input requirements; how legacy comparisons attribute every difference |
| `r1_planning_record.md` | Archived R-1 planning plan with its revision log and evidence ledger. **Not authoritative** — `interfaces_decision.md` and `conventions.md` supersede it |
