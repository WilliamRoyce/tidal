...
\begin{enumerate}
  ...
  \item \textbf{Cumulative (secular growth)}: $\varepsilon \cdot \omega^2 \cdot t_{\mathrm{end}} \ll 1$. Controls the linear-in-time accumulation of the $O(\varepsilon^2)$ truncation error.
\end{enumerate}
The TIDAL validity monitor computes $\max_{\mathrm{modes}}(\varepsilon \cdot |\lambda|^2 \cdot t_{\mathrm{end}})$ from the Pass 0 eigendata and warns when the result exceeds $0.1$ (10\% accumulated error) or $1.0$ (full EFT breakdown).
