...
The zero-crossing period-measurement method used by \texttt{\_measure\_omega} has intrinsic precision $\Delta \omega / \omega \sim \Delta t / t_{\mathrm{end}}$ (roughly), where $\Delta t$ is the snapshot spacing. For $\omega \sim k$, $k = 2$, $t_{\mathrm{end}} = 40$, $N_{\mathrm{snapshots}} = 801$, this gives $\Delta\omega/\omega \sim 10^{-3}$. For small $n^2 - 1 \sim 4\rho B_0^2 \sim 10^{-3}$ (at $\rho = 10^{-3}$), the resulting absolute noise on $n_\perp^2 - 1$ is comparable to the signal, which limits the ratio measurement precision to few-\% at small $\varepsilon$. The ratio test above uses larger $\rho = 10^{-2}$ (where the signal is 10$\times$ noise) together with longer integration ($t_{\mathrm{end}} = 80$, $N_{\mathrm{snapshots}} = 1601$) and the exact finite-$\varepsilon$ correction to achieve clean agreement with $7/4$.
