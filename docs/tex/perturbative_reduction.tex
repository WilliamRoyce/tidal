...
\end{equation}
with QED-computed coefficients $c_1 = \alpha^2/(90\pi^2 m_e^4)$ and $c_2 = 7\alpha^2/(360\pi^2 m_e^4)$, giving $c_2/c_1 = 7/4$. The famous QED birefringence prediction is~\cite{Adler1971}
\begin{equation}
...
\begin{itemize}
  \item TIDAL's EH Lagrangian Eq.~\eqref{eq:eh-tidal-lagrangian} with $\sigma/\rho = 7/16$ (Eq.~\eqref{eq:eh-convention-mapping}) reproduces the Dunne/Adler QED birefringence ratio $7/4$ within the zero-crossing measurement precision.
  \item The convention mapping Eq.~\eqref{eq:eh-convention-mapping} is derived from the Lagrangian translation $\varepsilon^{\alpha\beta\gamma\delta}F_{\alpha\beta}F_{\gamma\delta} = 2 F\cdot\tilde{F}$; naive identification $\sigma/\rho = c_2/c_1$ is incorrect and yields ratio $7 \neq 7/4$. This is a convention difference, not a physics gap. Future EH-heavy work should either apply the mapping explicitly, or adopt the halved-dual convention by adding a $1/4$ factor in the Lagrangian \texttt{expression} to normalise the input.
  \item The test suite is structured so that neither $\sigma/\rho$ nor the target $7/4$ is tuned to match the other; both are independent inputs from the Lagrangian mapping and from~\cite{Adler1971,Dunne2004} respectively.
\end{itemize}
