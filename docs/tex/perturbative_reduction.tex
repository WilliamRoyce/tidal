...
\begin{itemize}
  ...
  \item \textbf{Quartic EM invariants} (Euler--Heisenberg $(F_{\mu\nu}F^{\mu\nu})^2$): supported as of issue~\#271 fix. During component decomposition, \verb|ExpandScalarWrappers| internally expands \verb|Power[F·F, 2]| → \verb|F·F * F·F| reusing the same abstract dummy index names, triggering xAct's \verb|Validate::repeated + Throw[Null]|. The fix extends the existing \verb|ValidateIndices = (True &)| suppression (already used for the $\tilde{R}^2$ component loop) to also cover: (a)~the pre-loop scalar-field path (\verb|ComponentDecompose.wl| scalar branch), and (b)~the hoisted pre-loop tensor-field path. The suppression only prevents false-positive validation throws; it does not alter the algebra. The standard torsion path is unaffected because $\tilde{R}^2$ uses \verb|RicciScalarCDT[]^2|, a power of a named zero-rank scalar (no abstract indices), and their \verb|Scalar[...]| contents contain CDs, routing them to the per-component path instead of the hoisted path.
\end{itemize}
