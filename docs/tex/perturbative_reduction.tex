...
\begin{description}
  ...
  \item[\texttt{TestEHPerturbativeEnergy}] Hamiltonian energy measurement on PerturbativeSolver output using the new \texttt{order\_in\_eps} infrastructure. \texttt{test\_order1\_h\_ratio\_matches\_adler}: measures $H_{\mathrm{order-1}}(y_0^{(0)}, t{=}0) / H_{\mathrm{order-0}}(y_0^{(0)}, t{=}0)$ and compares to $-3(n_\perp - 1)_{\mathrm{Adler}} = -6\rho B_0^2$ at QED parameters $\rho = 4\alpha^2/45$; a genuinely non-circular test exercising the Wolfram Hamiltonian path independently of the EOM path. \texttt{test\_energy\_conserved\_to\_eps\_squared}: expected to fail (\texttt{xfail}) until \#312 (spectral Parseval gradient energy for periodic domains) is implemented; the FD IBP gradient-energy systematic (${\sim}0.23\%$) dominates the QED $\varepsilon^2 \approx 2\times10^{-11}$ bound.
\end{description}
