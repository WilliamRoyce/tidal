...
\paragraph{Field-filtered energy (v0.22.8).}
As a practical workaround, \texttt{compute\_energy\_timeseries()} accepts an optional \texttt{fields} parameter that restricts the Hamiltonian evaluation to terms involving only the specified base fields. This is critical for Ostrogradsky theories where some fields' equations contain unresolvable $\partial_t^2$/$\partial_t^3$ coupled acceleration terms. For example, measuring conversion between $h_\times$ and $a_y$ on the combined PGT+EM system succeeds ($P = 0.997$) by passing \texttt{fields=\{h\_5, gea\_1\}}, even though the full 23-field Hamiltonian is ill-conditioned.
