...
\subsubsection{Position-Dependent + Periodic Auto-Routing (GH \#367)}
\label{sec:modal-pos-dep-auto-route}

\textbf{Important exception to the eligibility rules above.} Theories with
\emph{real spatial variation} in their position-dependent coefficients
(e.g.\ the Gertsenshtein dual-Gaussian background field, where
$B(x) = B_{\mathrm{peak}}[e^{-(x-z_{c1})^2/2\sigma^2} - e^{-(x-z_{c2})^2/2\sigma^2}]$)
build a Fourier convolution matrix
\[
A[m, m'] = \widehat{c}(m - m') \cdot i\,k_{m'},
\]
implemented in \texttt{\_add\_convolution\_coupling}
(\texttt{modal.py}:1587+). This matrix is \emph{intrinsically non-normal}
(the gradient multiplier $i k_{m'}$ is applied column-wise while the
convolution kernel mixes rows), and at finite $N$ it has spurious
positive-real-part eigenvalues that track the Nyquist wavenumber
$k_{\max} = \pi / \Delta x$ \emph{exactly}.  Empirical verification
(2026-05-18, E.0 dual-Gaussian theory at $L=100$):
\begin{center}
\begin{tabular}{rrrr}
$N$ & $n_{\mathrm{total}}$ & $\max\Re(\lambda)$ & $k_{\max} = \pi N / L$ \\
\hline
32  & 198  & 1.005  & 1.005  \\
64  & 396  & 2.011  & 2.011  \\
128 & 780  & 4.021  & 4.021  \\
256 & 1548 & 8.042  & 8.042  \\
512 & 3084 & 16.085 & 16.085 \\
\end{tabular}
\end{center}
The spurious eigenvalue \emph{grows linearly with grid resolution}, so
``use higher $N$'' is not a fix. Above $k_{\max} \cdot t_{\mathrm{end}}
\approx 2$ the artifact begins to dominate the dynamics in any
matrix-exponential method that materialises $\exp(A t)$ first;
above $k_{\max} \cdot t_{\mathrm{end}} \approx 20$ the result is wrong by
orders of magnitude (verified for dense Padé, \texttt{expm\_multiply}, and
a hand-coded Arnoldi-Krylov implementation).

The phenomenon is the non-normal pseudospectral transient growth of
Trefethen and Embree (\textit{Spectra and Pseudospectra}, 2005, Ch.\ 14--15),
inseparable from the discretization: $\partial_x \cdot B(x)$ has
eigenvalues $i k$ (all imaginary) in the continuous limit, but its
finite-$N$ Fourier representation acquires real-part eigenvalues that
$\exp(A t) \cdot v$ then amplifies.

\paragraph{Auto-routing under \texttt{--scheme auto}.} To prevent silent
wrong answers, the CLI \emph{auto-routes} all
position-dependent + periodic theories away from modal under
\texttt{--scheme auto} (via
\texttt{\_override\_pos\_dep\_periodic\_scheme} in
\texttt{tidal/cli/\_simulate.py}, just after \texttt{\_resolve\_scheme}
returns). The replacement depends on whether the theory carries algebraic
constraints (\texttt{time\_order=0} equations):
\begin{itemize}
  \item \textbf{No constraints} $\to$ \textbf{CVODE} (default). Faster
    than IDA; correct in physical space.
  \item \textbf{Has constraints} $\to$ \textbf{IDA}. CVODE would
    silently freeze the constraint fields at their initial values
    (\texttt{cvode.py}:103--104,
    \texttt{\_setup.py:warn\_frozen\_constraints} only warns), giving
    wrong physics for DAE systems. Phase E's localised PGT theories
    (T2--T6 with Erf-profile backgrounds) carry hidden mass-matrix
    constraints from the Schur path (\texttt{modal.py}:541--595); IDA
    handles those natively.
\end{itemize}
Both physical-space solvers avoid the Fourier convolution matrix entirely
and give correct bounded results. Users see a one-line notice:
\begin{lstlisting}[style=bash]
Note: position-dependent + periodic theory detected; auto-routing
to CVODE for correctness (GH #367 ...). Override with --scheme modal
if you have a known-safe parameter regime.
\end{lstlisting}
or, for the constraint case:
\begin{lstlisting}[style=bash]
Note: position-dependent + periodic theory with algebraic constraints
detected; auto-routing to IDA (GH #367 -- modal solver's
Fourier-convolution matrix has discretization artifacts; CVODE would
freeze the constraints at IC). Override with --scheme modal if you
have a known-safe parameter regime.
\end{lstlisting}

\paragraph{Override and safety net.} Explicit \texttt{--scheme modal}
runs the modal path normally. The post-evolution amplitude-growth check
in \texttt{\_evolve\_full\_matrix} (commit a2cdaa5, threshold
$10^6 \times$ initial amplitude) catches catastrophic cases and raises
\texttt{SimulationDivergedError} with the \texttt{--scheme cvode}
workaround in the message. The artifact is most severe at the
$k_{\max} \cdot t_{\mathrm{end}} \approx 40$ regime that triggers GH \#367.

\paragraph{IDA performance is acceptable for Phase E (measured 2026-05-18).}
Profiling the auto-route fallback on the GH \#367 reproducer (E.0
dual-Gaussian, no constraints) at the campaign envelope showed IDA wall
time within 1.3$\times$ of CVODE:
\begin{center}
\begin{tabular}{rrrr}
\toprule
$N$ & CVODE & IDA & IDA / CVODE \\
\midrule
64 & 1.24 s & 1.23 s & 0.99$\times$ \\
128 & 2.09 s & 2.62 s & 1.25$\times$ \\
\bottomrule
\end{tabular}
\end{center}
At PolyChord nested-sampling scale (10$^4$ likelihood calls per chain),
this puts the IDA-fallback path at $\sim$25 minutes additional per chain
vs an ideal modal-solver path -- tractable for Phase E. Adding actual
algebraic constraints to a derived PGT theory would add Newton-iteration
overhead but should remain well under the 3$\times$ threshold based on the
Stage A/B campaign timings for constant-$B_0$ PGT theories. Algorithm
work to fix the modal-solver discretization is therefore \emph{not} on
the Phase E critical path.

\paragraph{Future work} on the discretization itself: operator splitting
$A = A_{\mathrm{const}} + A_{\mathrm{conv}}$ with Strang or Yoshida-4 (the
codebase already distinguishes \texttt{term.position\_dependent} at
\texttt{modal.py}:1522); Hou-Li exponential filter on $\widehat{c}(q)$;
mixed real/Fourier representation that keeps the gradient in physical
space; higher-precision arithmetic as a reference oracle. None of these
were implemented in the GH \#367 fix or the Phase 1 followup; deferred
until a future Phase E measurement contradicts the timing above.
