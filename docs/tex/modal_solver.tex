...
\paragraph{Future work} on this issue (the discretization rather than its
workaround): operator splitting $A = A_{\mathrm{const}} + A_{\mathrm{conv}}$
with Strang or Yoshida-4 (the codebase already distinguishes
\texttt{term.position\_dependent} at \texttt{modal.py}:1522);
Hou-Li exponential filter on $\widehat{c}(q)$; mixed
real/Fourier representation that keeps the gradient in physical space;
higher-precision arithmetic as a reference oracle. None of these were
implemented in the GH \#367 fix; see the planning notes for the next
investigation pass.
