...
position-dependent + periodic theories to CVODE under \texttt{--scheme
auto} (\texttt{tidal/cli/\_simulate.py}, just after \texttt{\_resolve\_scheme}
returns). CVODE integrates in physical space, never builds the Fourier
convolution matrix, and gives correct bounded results. Users see a one-line
notice:
\begin{lstlisting}[style=bash]
