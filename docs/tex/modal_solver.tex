...
For the PGT graviton-torsion system, this reduces 17 fields (after Ostrogradsky) to 14 dynamical slots (7 fields $\times$ 2), with 10 constraint field eliminations, 2 mass-matrix constraints, and several velocity-coupling gauge modes:

\begin{center}
\begin{tabular}{@{}lrl@{}}
\toprule
Stage & Fields/slots & Eliminated \\
\midrule
After Ostrogradsky & 17 fields, 24 slots & --- \\
Level 1 (mass-matrix) & 14 slots remain & $t_3 = t_7$, $w_{h_3}/w_{h_5}$ relation \\
Level 2 (constraint Schur) & 14 dyn slots & 10 constraint fields \\
Level 3 (QZ velocity) & Physical DOF & Gauge directions frozen \\
Eigendecomposition & $\exp(\lambda \cdot t)$ & $O(1)$ time evolution \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Implementation}: \texttt{\_build\_generalized\_evolution\_matrices()} returns $A_\text{rhs}$ and optionally $B = I - V_\text{coupling}$. When $B$ is provided, \texttt{\_evolve\_per\_mode()} uses \texttt{scipy.linalg.eig(A, B)} instead of \texttt{np.linalg.eig(A)}. Backward compatible: for systems without time-derivative operators, the existing code paths are used unchanged.
