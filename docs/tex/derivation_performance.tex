...
\texttt{torsion\_gertsenshtein} (minimal $\tilde{R}^2$) & ${\sim}1$m\,53\,s  & 3 & 20 & post-\#246, was ${\sim}20$\,min \\
\texttt{torsion\_gertsenshtein} (non-minimal) & ${\sim}2$m\,38\,s  & 3 & 20 & post-\#246, was ${\sim}6$\,min \\
\texttt{torsion\_gertsenshtein} (propagating) & ${\sim}2$m\,50\,s  & 3 & 20 & post-\#246, was ${\sim}8$\,min \\
\texttt{graviton\_torsion} (minimal propagating) & ${\sim}1$m\,23\,s  & 2 & 13 & post-\#246, was ${\sim}2$\,min \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Known-stale headers --- do not trust at face value.} Several \texttt{examples/*/theory*.toml} files carry timing comments that predate the parity-odd / per-pair-TraceBasisDummy optimisation series (issue~\#246, commits \texttt{18dfa9a}--\texttt{a28a393}, 2026-04-11). Those optimisations dropped the R$^2$/\(\tilde{R}^2\) family by $3\text{--}11\times$. Until each of these headers is re-measured and rewritten, the \emph{header} number in the source file should be considered a \emph{ceiling}, not a current measurement:
\begin{itemize}
  \item \texttt{examples/torsion\_gertsenshtein/theory\_b5\_zero.toml} --- header claimed ``15--18 min''; post-\#246 the simpler $b_5 = 0$ limit is strictly faster than the full \texttt{theory.toml} ($\approx 2$ min post-\#246), so expect $\le 2$ min on first run.
  \item \texttt{examples/torsion\_gertsenshtein/theory\_minimal\_propagating.toml} --- header updated in v0.33.9 to \texttt{a28a393}'s $\approx 1$ m $23$ s measurement.
  \item \texttt{examples/euler\_heisenberg/theory.toml} --- header updated in v0.33.9 to the $\approx 19$~s measurement first recorded at v0.33.9 (the theory was unable to derive at all before that release; see \cref{sec:pr-eh-cd}).
\end{itemize}
Theories whose headers match the post-\#246 table above have been re-verified against the \texttt{a28a393} commit log or re-measured in the v0.33.9 regression suite. If a header is absent or says ``to be verified'', run the derivation once and annotate.
