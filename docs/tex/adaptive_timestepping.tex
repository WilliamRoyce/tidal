...
supported Fourier operators. Position-dependent coefficients --- RHS $c(x)$ and
kinetic $M(x)$ alike --- are handled via $k$-space convolution in the full FFT
basis (GH \#427/\#445); the per-mode engines used by the stability probe and
the Pass-1 correction path still require a constant $M$.
...
\label{sec:adaptive-timestepping-cli}
