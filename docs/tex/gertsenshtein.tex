...
\end{equation}
where $\kappa^2 = 16\pi G$, $R$ is the Ricci scalar, and $F_{\mu\nu} = \partial_\mu A_\nu - \partial_\nu A_\mu$ is the electromagnetic field strength tensor. Throughout TIDAL, $\kappa$ is the \emph{vertex coupling} (linearised graviton, $[\kappa] = [\text{Mass}]^{-1}$), \emph{not} the Einstein action constant $\kappa_\text{E} = 8\pi G$ of some BHL papers — the two differ by a factor of $\sqrt{2}$. See \cref{sec:kappa-conventions} for the full discussion.
...
\textbf{Lella et al.}~\cite{lella2024constraining} & \textbf{2024} & \textbf{4-component mixing, canonical graviton --- confirms our formula} \\
...
\textbf{Source}: Eigenmode analysis of coupled $h_7/a_2$ system; confirmed by Lella et al.~\cite{lella2024constraining}.
...
\textbf{Reference}: Raffelt \& Stodolsky~(1988, PRD 37:1237); Lella et al.~\cite{lella2024constraining}
...
the photon sector~\cite{barker2024poincare}.
...
\begin{enumerate}
  \item \textbf{Plasma detuning} (Phase F1): \textbf{Implemented in v0.30.1}. The perturbation-level Proca term $-(m_A^2/2) a_\mu a^\mu$ is applied directly to the $O(\varepsilon)$ photon via \texttt{xPert}, avoiding the gauge-potential coupling artefact that blocked the earlier full-field approach. Validated against Raffelt \& Stodolsky 1988 (HWHM 0.405 vs theory 0.402 at $B_0=0.10$, commit \texttt{1526d77}). See \cref{sec:gertsenshtein-proca} and \texttt{examples/gertsenshtein\_proca/}.
  ...
  \item \textbf{Axion--photon--graviton mixing} (future): Three-way mixing matrix with $g_{a\gamma\gamma}\, a\, F\, \tilde{F}$. Ref: Lella et al.~\cite{lella2024constraining}.
