...
\subsubsection{Results (v0.24.3)}

\begin{table}[htbp]
\centering
\caption{Dark photon torsion model: conversion coefficient $C_0 = P/B_0^2$ at
  $t = 5$, $B_0 = 0.01$, $\kappa = 1$.}
\label{tab:dark-photon-results}
\begin{tabular}{@{}llll@{}}
\toprule
Scenario & Parameters & $C_0$ & Notes \\
\midrule
Standard Gertsenshtein & $\xi = 0$, $\delta = 0$ & $6.25 \times 10^{-4}$  & Exact formula $P = \sin^2(\kappa B_0 t/2)$ \\
Nominal torsion model & $\alpha=0.5$, $\xi=0.1$, $\delta=0.1$ & $7.24 \times 10^{-4}$ & +16\% amplification \\
\bottomrule
\end{tabular}
\end{table}

\begin{itemize}
  \item \textbf{$\xi = 0$ recovery:} At $\xi = 0$ the torsion kinetic term vanishes;
    the torsion fields become algebraic constraints; the conversion probability
    exactly recovers the standard Gertsenshtein formula.  This is the primary
    control check for the model (verified, commit~\texttt{6627b21}).
  \item \textbf{$B_0$ scaling:} $C_0 = P/B_0^2$ verified constant to $<1\%$ across
    $B_0 \in \{0.003,\, 0.01,\, 0.03\}$, confirming the linear regime.
  \item \textbf{Energy conservation:} $\max |dE/E| = 1.07 \times 10^{-15}$
    (machine precision, Fourier modal solver).
\end{itemize}
