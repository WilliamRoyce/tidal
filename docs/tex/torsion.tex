...
Example equation structure (schematic):
\begin{equation}
  \partial_t^2 h_0 = c_1\, \nabla_x^2 h_5 + c_2\, \partial_x^3 t_0 - c_3\, \partial_x^3 t_8 + c_4\, \nabla^4 h_0 + \cdots
\end{equation}
where the $\partial_x^3 t$ terms confirm graviton-torsion mixing, and the $\nabla^4 h$ terms arise from $\tilde{R}^2$.

The canonical Hamiltonian contains 14 terms with $b_5$-dependent expressions, verified to be correct by comparison with the Lagrangian structure.

\paragraph{After Ostrogradsky reduction.}
Fields with 4th-order-in-time equations acquire auxiliary fields, expanding the state from 15 to 17 fields (2 auxiliary). All equations become $\leq$2nd order in time.
...
\begin{table}[htbp]
\centering
\caption{Torsion implementation status (as of v0.16.1).}
\label{tab:torsion-status}
\begin{tabular}{@{}llp{7cm}@{}}
\toprule
Component & Status & Notes \\
\midrule
Symbolic derivation & Complete & Component E--L: 15 equations in $\sim$5\,s (vs 77\,min VarD) \\
JSON specification & Complete & 15 fields (6 graviton $h_{0\text{--}5}$ + 9 torsion $t_{0\text{--}8}$) \\
Ostrogradsky reduction & Complete & $h_3, h_5$ (4th-order) $\to$ auxiliaries $w_{h_3}, w_{h_5}$; 17 fields \\
Mass-matrix analysis & Complete & $M = [[1,1],[1,1]]$ for $\{t_3, t_7\}$: hidden constraint $t_3 = t_7$ \\
Simulation (modal) & \textbf{Complete} & Generalized eigenvalue via QZ; $\max|dE/E| = 0$ at $t=100$ \\
Energy conservation & \textbf{Pass} & Machine precision ($0$) for all tested $t_\text{end}$ \\
Parameter sweeps & Not tested & Requires physical parameter selection \\
\bottomrule
\end{tabular}
\end{table}
...
The 2+1D graviton-torsion system has 15 component fields:

\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
Tensor & Components & Fields \\
\midrule
Metric perturbation $h_{\mu\nu}$ (symmetric) & 6 & $h_0{=}h_{00},\ h_1{=}h_{01},\ h_2{=}h_{02},\ h_3{=}h_{11},\ h_4{=}h_{12},\ h_5{=}h_{22}$ \\
Torsion perturbation $t_{\alpha\mu\nu}$ (antisym. in $\mu\nu$) & 9 & $t_0{=}t_{001},\ t_1{=}t_{002},\ \ldots,\ t_8{=}t_{212}$ \\
\bottomrule
\end{tabular}
\end{center}

After Ostrogradsky reduction: $h_3, h_5$ (4th-order from $\tilde{R}^2$) $\to$ auxiliaries $w_{h_3}, w_{h_5}$, giving 17 fields total. 10 are constraints (time\_order=0), 5 are 2nd-order dynamical (after mass-matrix elimination), with 2 mass-matrix constraints ($t_3 = t_7$, $w_{h_3}/w_{h_5}$ relation).

\subsubsection{Remaining Validation Tasks}

\begin{itemize}
  \item Conversion measurement between $h$ and $T$ components (no analytical formula exists; requires numerical sweep)
  \item Dispersion relation $\omega(k)$ comparison with Nikiforova et al.\ (2009) stability analysis
  \item Propagating mode count cross-check with PSALTer / Barker (2024) predictions
  \item Background $B$ field parameter sweeps for graviton-torsion mixing (\#154)
\end{itemize}
