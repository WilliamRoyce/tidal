...
\begin{table}[htbp]
\centering
\caption{Torsion implementation status (as of v0.15.3).}
\label{tab:torsion-status}
\begin{tabular}{@{}llp{7cm}@{}}
\toprule
Component & Status & Notes \\
\midrule
Symbolic derivation & Complete & Component E--L produces 15 correct equations in $\sim$5\,s (2+1D) \\
JSON specification & Complete & 26 equations, 15 fields, correct operator/coefficient structure \\
Ostrogradsky reduction & Complete & Automatic on JSON load; 15 $\to$ 17 fields, all $\leq$2nd order \\
Hamiltonian terms & Complete & 14 terms with $b_5$ expressions, verified correct \\
\textbf{Simulation} & \textbf{Blocked} & Implicit $\partial_t^2$/$\partial_t^3$/mixed operators on 2nd-order fields (\#165) \\
Energy conservation & Not tested & Blocked by simulation \\
Parameter sweeps & Not tested & Blocked by simulation \\
\bottomrule
\end{tabular}
\end{table}

The simulation blocker (\#165) arises because some equations contain implicit references to accelerations of other 2nd-order fields. For example, $\partial_t^2(t_3)$ appears in $t_7$'s equation---this means ``the acceleration of $t_3$'', which is itself defined by $t_3$'s own equation of motion. Resolving this requires either:
\begin{enumerate}
  \item \textbf{IDA residual formulation:} Treat these as implicit DAE constraints.
  \item \textbf{Explicit RHS substitution:} Replace $\partial_t^2(q_j)$ with $\text{RHS}_j$ during Ostrogradsky reduction.
  \item \textbf{Extended Ostrogradsky:} Introduce auxiliaries for all cross-referenced fields.
\end{enumerate}
