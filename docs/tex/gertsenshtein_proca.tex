...
detuning $\Delta m^2 = m_A^2 - m_g^2_{\mathrm{eff}}$ that suppresses the
...
\begin{itemize}
  ...
  \item \textbf{Domcke, Garcia-Cely \& Lee 2025},
    ...
    $[\Box - \mu^2]A_h = -j_{\mathrm{eff}}$, equivalent to the
    Lagrangian approach used here via variational calculus.
...
\begin{equation}
  \mathcal{H}_{\mathrm{mix}} = \begin{pmatrix}
    m_g^2_{\mathrm{eff}} & 2\kappa B_0 k \\
    2\kappa B_0 k & m_A^2
  \end{pmatrix}
  \label{eq:mixing-hamiltonian}
\end{equation}
where $m_g^2_{\mathrm{eff}} = \kappa^2 B_0^2 / 2$ is the graviton effective
...
\begin{equation}
  ...
  \qquad \Delta m^2 = m_A^2 - m_g^2_{\mathrm{eff}}.
  \label{eq:lorentzian}
\end{equation}
...
treatment via variational calculus, and uses the curved-spacetime Maxwell
field conventions of Hwang \& Noh 2023.
...
\begin{enumerate}
  ...
  \item \textbf{Axion-photon-graviton mixing} (Berlin et al.\ 2024,
    ...
    the two-state mixing matrix to three states. The numerical methods
    in Berlin et al.\ are directly analogous to TIDAL's approach.
\end{enumerate}
