...
The reduction is transparent to users---it happens automatically in \texttt{ostrogradsky.py} during JSON loading. For the 2+1D graviton-torsion example, this converts 15 original fields to 17 fields (2 auxiliary), with all equations at time order $\leq 2$.

\paragraph{Implicit acceleration coupling.}
After reduction, equations may contain implicit references to accelerations of other fields (e.g., $\partial_t^2 q_j$ in $q_i$'s equation). The generalized mass-matrix modal solver (v0.16.0) handles this via operator decomposition, singular mass-matrix eigendecomposition, and QZ generalized eigenvalue for velocity coupling. See \cref{sec:modal-mass-matrix} for the complete algorithm. Issue \#165 resolved.
