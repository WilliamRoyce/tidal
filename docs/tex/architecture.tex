...
\begin{figure}[htbp]
\centering
\input{../figures/solver_flowchart}
...
\subsection{Working Examples (27 Total)}
...
\midrule
\texttt{scalar\_field/} & 1+1D & phi\_0 & Basic scalar wave, mass term \\
\texttt{electromagnetic/} & 1+1D & A\_0, A\_1 & Maxwell equations, Lorenz gauge \\
\texttt{proca/} & 1+1D & A\_0, A\_1 & Massive vector field (Proca mass) \\
\texttt{coupled\_scalars/} & 1+1D & phi\_0, chi\_0 & Cross-field coupling, mass matrix \\
\texttt{scalar\_potential\_well/} & 1+1D & phi\_0 & Background potential well, bound states \\
\texttt{cylindrical\_kg\_1d/} & 1+1D & phi\_0 & Cylindrical coordinates, plane-wave 1D reduction \\
\texttt{gravitational\_waves\_1d/} & 1+1D & h\_ij & Linearized gravity, plane-wave 1D reduction \\
\texttt{spherical\_kg\_1d/} & 1+1D & phi\_0 & Spherical coordinates, plane-wave 1D reduction \\
\texttt{chern\_simons/} & 2+1D & A\_0, A\_1, A\_2 & Epsilon tensor, constraint equation \\
\texttt{elasticity/} & 2+1D & u\_0, u\_1 & Anisotropic Laplacian, cross derivatives \\
\texttt{curved\_spacetime/} & 2+1D & phi\_0 & De Sitter Hubble friction, time-dependent coefficients \\
\texttt{sphere\_kg/} & 2+1D & phi\_0 & Stereographic $S^2$, position-dependent coefficients \\
\texttt{polar\_kg/} & 2+1D & phi\_0 & Polar coordinates, $1/r$ Christoffel correction \\
\texttt{electrostatics/} & 2+1D & phi\_0 & Poisson equation, constraint solver \\
\texttt{scalar\_vector\_coupling/} & 2+1D & phi\_0, A\_i & Mixed-rank cross-field (scalar+vector), 4 constants \\
\texttt{massive\_gravity/} & 2+1D & h\_ij & Linearized massive gravity, Fierz--Pauli, xPert~\cite{brizuela2009xpert} \\
\texttt{coupled\_proca/} & 2+1D & A\_i, B\_i & Two massive vectors, coupled Helmholtz constraints \\
\texttt{coupled\_scattering/} & 2+1D & phi\_0, chi\_0 & Position-dependent Gaussian coupling, background fields \\
\texttt{proca\_background/} & 2+1D & A\_i, B\_i & Lorentzian scalar background, constraint+BG integration \\
\texttt{vector\_background/} & 2+1D & phi\_0, A\_i & Tanh domain wall vector background, sign-changing coupling \\
\texttt{scalar\_field\_3d/} & 3+1D & phi\_0 & Full 4D Klein--Gordon, $32^3$ grid \\
\texttt{spherical\_kg/} & 3+1D & phi\_0 & Spherical coordinates, trig coefficients (Cot, Csc) \\
\texttt{cylindrical\_kg/} & 3+1D & phi\_0 & Mixed curved $(r,\theta)$ and flat $(z)$ directions \\
\texttt{gravitational\_waves/} & 3+1D & h\_ij & xPert~\cite{brizuela2009xpert} linearization, TT gauge, rank-2 tensor \\
\texttt{massive\_3form/} & 3+1D & C\_ijk & Rank-3 antisymmetric tensor, symmetry reduction $64 \to 4$ \\
\texttt{gertsenshtein/} & 1+1D & h\_5, gea\_1 & Einstein--Maxwell graviton-photon conversion, multi-field perturbation \\
\texttt{maxwell\_perturbation/} & 1+1D & gea\_0, gea\_1 & Matter-only Proca perturbation, lightweight CI test \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Ostrogradsky Reduction}
\label{sec:ostrogradsky}

Higher-derivative Lagrangians (e.g., $R^2$, $\tilde{R}^2$ curvature-squared terms) produce equations of motion with time derivatives of order $>2$. Since all PDE solver backends (IDA, CVODE, modal, leapfrog) support at most second-order-in-time equations, an automatic \textbf{Ostrogradsky reduction} is applied when the JSON specification is loaded.

\paragraph{Algorithm.}
For each field $q$ whose equation has time order $n_t > 2$:
\begin{enumerate}
  \item Introduce an auxiliary field $w_q = d^2 q / dt^2$ (the acceleration).
  \item The original equation $d^{n_t} q/dt^{n_t} = \text{RHS}[q, \ldots]$ becomes $d^{n_t - 2} w_q / dt^{n_t - 2} = \text{RHS}[q, \ldots]$.
  \item Add a constraint linking the auxiliary: $d^2 q / dt^2 = w_q$.
  \item Repeat until all equations have time order $\leq 2$.
\end{enumerate}
The state vector expands from $(q, v_q)$ to $(q, v_q, w_q, v_{w_q})$ for each reduced field.

\paragraph{Operator reduction map.}
Each higher-derivative operator acting on the original field $q$ is mapped to a lower-order operator acting on the auxiliary $w_q = \partial_t^2 q$:

\begin{table}[htbp]
\centering
\caption{Ostrogradsky operator reduction: original operators on $q$ are rewritten as operators on auxiliary $w_q$.}
\label{tab:ostrogradsky-operator-map}
\begin{tabular}{@{}lll@{}}
\toprule
Original on $q$ & Reduced on $w_q$ & Meaning \\
\midrule
$\partial_t^4 q$ & $\partial_t^2 w$ & $\partial_t^4 q = \partial_t^2(\partial_t^2 q) = \partial_t^2 w$ \\
$\partial_t^3 q$ & $\partial_t w$ & $\partial_t^3 q = \partial_t(\partial_t^2 q) = \dot{w}$ \\
$\partial_t^2 q$ & $w$ & $\partial_t^2 q = w$ (defining relation) \\
$\partial_t^2 \partial_x q$ & $\partial_x w$ & spatial derivative passes through \\
$\partial_t^3 \partial_x q$ & $\partial_t \partial_x w$ & combined: reduce time order by 2 \\
\bottomrule
\end{tabular}
\end{table}

The general rule: time order decreases by 2, spatial derivatives are preserved unchanged.

\paragraph{Cross-field substitution.}
When $\partial_t^2(q_j)$ appears on the RHS of $q_i$'s equation and $q_j$ is itself a 4th-order field, the reference is rewritten as $w_{q_j}$ (the identity operator on the auxiliary). This resolves implicit acceleration cross-references \emph{before} the modal solver's mass-matrix analysis. For terms where the referenced field is 2nd-order (not reduced), the $\partial_t^2$ operator remains and contributes to the off-diagonal mass matrix $M$.

\paragraph{Ghost stability.}
Ostrogradsky's theorem (1850): non-degenerate higher-derivative Lagrangians generically produce unbounded Hamiltonians. The auxiliary field $w$ carries opposite-sign kinetic energy, creating \emph{ghost instabilities} with exponentially growing modes. For PGT, ghost-free parameter windows exist where the Hamiltonian remains bounded (Nikiforova et al.\ 2009; Barker 2024, arXiv:2406.12826). TIDAL does not enforce these constraints automatically; a runtime warning is emitted about potential ghost instability. Full ghost propagator analysis is tracked in issue \#164.

\paragraph{Integration.}
The reduction is transparent to users---it happens automatically during JSON loading. For the 4D graviton-torsion example (v0.18.0), this converts 34 original fields to 37 fields (3 auxiliary: $w_{h_4}$, $w_{h_7}$, $w_{h_9}$), with all equations at time order $\leq 2$.

\paragraph{Implicit acceleration coupling.}
After reduction, the off-diagonal mass matrix $M$ may be \emph{singular} (e.g., PGT Poincar\'e gauge symmetry creates $M = \bigl[\begin{smallmatrix} 1 & 1 \\ 1 & 1 \end{smallmatrix}\bigr]$ for coupled torsion components). The generalized mass-matrix modal solver (v0.16.0) handles this via eigendecomposition of $M$ to detect hidden constraints, jerk ($\dddot{x}$) substitution, and QZ generalized eigenvalue for circular velocity coupling. See \cref{sec:modal-mass-matrix} for the complete three-level elimination algorithm.
