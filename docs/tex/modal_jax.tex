...
\paragraph{Phase 2a (constraint systems) — implemented.}
...
\begin{itemize}
  ...
  \item \texttt{dark\_photon\_plasma} ($N=32$, 17~modes, 30~dyn slots,
    Phase~2a+2b): scipy~$11$ms, JAX~$21$ms, speedup~$0.51\times$
    (machine-precision agreement, but \textbf{JAX is slower at this size}).
\end{itemize}
...
\begin{verbatim}
dark_photon_plasma N=32 breakdown (Phase 2b):
  _build_evolution_matrices (numpy Schur):  1.94 ms
  batch_expm (Padé, 17 modes x 30x30):      8.76 ms  <- dominant
  evolve_uniform scan:                      0.08 ms
  einsum recovery + IFFT + assembly:        ~10  ms
  Total JAX:                                ~21  ms

  scipy total (Padé loop + IFFT):           ~11  ms
\end{verbatim}
