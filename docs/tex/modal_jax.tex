...
The following paths raise \texttt{NotImplementedError} in phase~1 and fall
back to \texttt{--scheme modal}:
...
\begin{itemize}
  ...
  \item Algebraic constraint fields (Schur elimination path,
    \texttt{\_build\_evolution\_matrices}).  Requires generalised
    eigendecomposition with rank-deficient mass matrix.
...
\paragraph{Local benchmark.}
Run \texttt{scripts/local\_jax\_benchmark.py} to measure the achieved
speedup on the development machine.  JIT compile time (first call) is
reported separately and excluded from the benchmark mean.  Relative error
is verified to be $<10^{-10}$ against the scipy backend.
...
\end{verbatim}
Do not submit until local benchmark results show $\geq3\times$ speedup and
