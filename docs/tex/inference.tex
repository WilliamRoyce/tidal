...
Nested sampling produces weighted posterior samples from which anesthetic
computes three key diagnostics:

\begin{description}
  \item[$D_\mathrm{KL}$] Kullback--Leibler divergence from prior to
    posterior (nats).  Measures total information gained.
  \item[$d_G$] Bayesian model dimensionality: the effective number of
    parameters constrained by the likelihood.  If $d_G \ll n_\mathrm{params}$,
    most parameters are prior-dominated.
  \item[Marginal $D_\mathrm{KL}$] Per-parameter KL divergence.  A
    parameter with high marginal $D_\mathrm{KL}$ is strongly constrained;
    one near zero is irrelevant to the likelihood.
\end{description}
...
$0.1$--$1$~nat = moderate, $< 0.1$~nat = weak.
