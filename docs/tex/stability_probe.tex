...
\subsubsection{Verification (TODO)}
\label{sec:ieee-floor-verification}

A direct numerical demonstration of the IEEE 754 FFT floor at the
$\sim 10^{-14}$ level on a $256$-point $\cos(k_0 x)$ transform, with
the corresponding $e^{\gamma_n t}$ amplification curve overlaid for
the D1 contamination $\gamma$ range, will be produced by a separate
diagnostic script. Once that figure lands, embed it here with the
\texttt{includegraphics} call and a short caption pinning the $128$-vs-$10^{-14}$
amplitude ratio and the implied minimum $\gamma_n t$ at which a
non-fundamental bin overtakes the fundamental.

\textbf{TODO}: replace this stub with
\verb|\includegraphics{...}| once the IEEE-floor diagnostic produces
the figure.
