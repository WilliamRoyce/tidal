...
\paragraph{Source-projection effect.} The simulation's per-bin h_5
amplitude grows at $\gamma_\mathrm{amp,\,h_5} = 7.08$, distinctly
below the probe's reported $\gamma_{v1} = 17.11$. This is not a
contradiction but the expected eigenvector-projection effect: the
fastest-growing eigenvector in the source-containing block lives
mostly in the torsion sector ($t_X$ slots) and projects only weakly
onto the source $h_5$ slot. The block 2-norm
$\sqrt{\sum_i |\hat\Phi_i(k)|^2}$ across all fields in the block
grows at $\gamma_{v1}$, which is why both the probe (operating on
the unit-IC at the source slot in the reduced system) and the
simulation's pre-flight divergence guard (operating on the same
reduced-system block norm) correctly catch this sample. A
single-field projection would under-estimate the threat.
