...
\begin{itemize}
  ...
  \item The $\delta_1$ coupling is the simplest non-minimal torsion--EM interaction from the general PGT+EM enumeration~\cite{shapiro2002torsion,bahamonde2025torsion}.
...
$f(R)$ scalaron & Graviton amplitude grows & $\nu \to -1$ & \cite{cembranos2023graviton} \\
Axion--photon MSW & Conversion width diverges & $m_a = \omega_p$ & \cite{berlin2024axionphoton} \\
\bottomrule
\end{tabular}
\end{center}
...
In physical units, $\kappa = \sqrt{16\pi G/c^4} = \sqrt{2}/M_{\text{Pl}}$ where $M_{\text{Pl}} = 2.44 \times 10^{18}\,\text{GeV}$ is the reduced Planck mass with $M_{\text{Pl}}^2 = 1/(8\pi G)$. (See \cref{sec:kappa-conventions} for the disambiguation of $\kappa$ from the Einstein-action $\kappa_\text{E} = 8\pi G$ used in some BHL papers; note that earlier drafts of this section incorrectly stated $\kappa = 1/M_{\text{Pl}}$, missing the $\sqrt{2}$.) The torsion mass parameters $\alpha_i$ have dimensions of $[\text{length}]^2$ in natural units; in our convention ($\kappa = 1$), they are measured in units of $M_{\text{Pl}}^{-2}$. The coupling $\delta_1$ is dimensionless.
