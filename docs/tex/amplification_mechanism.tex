...
\subsubsection{Tachyonic Photon Transition}

The sharp amplification peak at the stability boundary is driven by the \textbf{effective photon mass transition}: the torsion Schur complement shifts the photon effective mass $m^2_{\text{eff}}(a_1)$ from negative (oscillatory) to positive (tachyonic). At the transition point ($m^2_{\text{eff}} = 0$), the system satisfies the resonance condition for maximal conversion.

At $\delta_1 = 1.0$, the transition occurs at $\alpha_2 \approx -0.912$:
\begin{itemize}
  \item $\alpha_2 < -0.912$: $m^2_{\text{eff}}(a_1) < 0$ (stable), coupling reversed ($\mu_{\text{eff}}/\mu_{\text{GR}} \approx -1.37$), conversion suppressed by detuning.
  \item $\alpha_2 \approx -0.909$: $m^2_{\text{eff}} \approx 0$ (resonance), maximum conversion $A \approx 665$.
  \item $\alpha_2 > -0.909$: $m^2_{\text{eff}} > 0$ (tachyonic), exponential photon growth $\to$ divergence.
\end{itemize}

The transition is extremely sharp: $A$ jumps from $0.19$ to $665$ over $\Delta\alpha_2 \approx 0.008$, consistent with a resonance-width scaling near the mass-zero crossing.

\subsection{Numerical Results}
\label{subsec:numerical-results}

Parameter sweeps with plane-wave initial conditions ($\Bzero = 10^{-4}$) give the following validated results:
\begin{itemize}
  \item \textbf{$\alpha_2$ high-resolution sweep} ($\delta_1 = 1.0$, 100 points): 37 valid ($P < 0.1$), 54 diverged. Maximum amplification $A = 665\times$ at $\alpha_2 = -0.909$. Suppression valley $A \approx 0.001$ across most of the stable range.
  \item \textbf{$\Bzero$ scaling} ($\delta_1 = 1.0$, $\alpha_2 = -0.82$): $C_0 = P/\Bzero^2 = 30{,}204$ constant to 4 significant figures over $\Bzero = 10^{-6}$ to $10^{-4}$, confirming coupling modification ($A = 48$ at this parameter point).
\end{itemize}
...
\subsubsection{Enhanced Effective Coupling}

The peak amplification $A \approx 665$ at the stability boundary ($\delta_1 = 1$, $\alpha_2 = -0.909$) means the effective graviton--photon coupling is enhanced:
\begin{equation}
  \kappa_{\text{eff}} = \sqrt{A}\, \kappa \approx 26\, \kappa.
  \label{eq:kappa-eff}
\end{equation}
Equivalently, the minimum detectable magnetic field for graviton--photon conversion is reduced by $\sqrt{A} \approx 26$. Away from the boundary ($\alpha_2 = -0.82$), the amplification is more modest: $A \approx 48$, $\kappa_{\text{eff}} \approx 7\kappa$.

For gravitational wave backgrounds, the induced photon flux scales as~\cite{Lella2024}:
\begin{equation}
  \frac{dF_{g \to \gamma}}{df} \propto \kappa_{\text{eff}}^2\, \Bzero^2\, h_c^2(f) \propto A\, \kappa^2\, \Bzero^2\, h_c^2(f).
\end{equation}
Constraints on $h_c(f)$ from non-observation of conversion-induced photon flux~\cite{Lella2024} are thus strengthened by a factor $\sqrt{A}$ in the torsion-modified theory:
\begin{equation}
  h_c^{\text{torsion bound}} = \frac{h_c^{\text{GR bound}}}{\sqrt{A}}.
\end{equation}

Lella \emph{et al.}~\cite{Lella2024} derive constraints on $h_c$ at $f \sim 10^{9}$--$10^{26}\,\text{Hz}$ from the Galactic magnetic field (both regular and turbulent components), improving previous bounds by $1$--$2$ orders of magnitude. With torsion amplification at $A = 665$ (peak) or $A = 48$ (moderate coupling), these bounds would be further strengthened by a factor $\sqrt{A} \approx 7$--$26$, or conversely, the same bounds would constrain the torsion parameter space $(\delta_1, \alpha_i)$ by ruling out regions where $A$ exceeds the existing GR limits.

\subsubsection{Spectral Signature}

The $k$-dependent enhancement (see~\cref{subsec:phenomenology}) produces a characteristic \textbf{blue-tilted spectral signature}: higher-frequency gravitational waves experience stronger conversion amplification. This is in contrast to:
\begin{itemize}
...
\end{itemize}
The blue tilt arises because the Schur complement correction grows with $k^2$ (spatial operators in the constraint equations). This spectral signature is a \textbf{unique prediction} of torsion-mediated graviton--photon coupling that could distinguish it from other beyond-Standard-Model modifications in broadband GW conversion searches.
