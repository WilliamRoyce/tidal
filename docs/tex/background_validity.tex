...
For Einstein--Maxwell, Hwang \& Noh~\cite{hwangnoh2023graviton} derive the
conversion probability
\begin{equation}
  P_{g\to\gamma}\sim P_{\gamma\to g}\sim 8\pi^{2}\sin^{2}\!\theta\,(L/\lambda_{B})^{2},
  \label{eq:bv-hn-P}
\end{equation}
where $\lambda_{B}\equiv 2\pi/k_{B}$ is the critical length scale for
tachyonic graviton instability and $(L/\lambda_{B})^{2}=(2/\pi)(L^{2}GB^{2}/c^{4})$
encodes the gravitational self-energy of the background magnetic field.
Inverting, the backreaction parameter
$(L/\lambda_{B})^{2}\sim P/(8\pi^{2}\sin^{2}\!\theta)$ is proportional to
the conversion probability~$P$ itself, hence negligible whenever
$P\ll 1$.  Hwang~\&~Noh further note that the opposing limit
$L > \lambda_{B}$ (equivalently $P\gtrsim 1$) ``implies the background
system is in fully relativistic stage \ldots\ violating the basic
assumption of the graviton--photon conversion analysis'', so the
exponential-instability / saturated-resonance regime lies \emph{outside}
the linearised Einstein--Maxwell treatment on which all these
calculations rest.  However, their analysis is specific to
Einstein--Maxwell and does not address torsion, non-minimal couplings,
or higher-order EFT operators.  This document provides a
\textbf{theory-independent} generalisation.
...
Simulations use natural units with $\kappa \equiv \sqrt{16\pi G} = 1$ (the vertex coupling — see \cref{sec:kappa-conventions} for the disambiguation from the Einstein-action $\kappa_\text{E} = 8\pi G$).
