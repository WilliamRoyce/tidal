...
| B.5-pub | D2.0–D2.3 v3 publication pass (50×ndim) | 🟡 in progress | Already done: D2.0 sup pub (29471255, logZ=+5.837±0.143, ESS=2404), D2.1 sup pub (29471255, logZ=+2.524±0.158, ESS=2895), **D2.0 amp pub** (29507332, logZ=+7.679±?, ESS=1063, ✅ complete). **D2.1 amp pub** (29507332, partial — 63% at SIGTERM, logZ=+5.32±0.17 reconstructed; needs resume via `run_pub_batch_d2amp_p2_resume.sh`). **D2.2 amp pub** (29511699, 🔄 running), **D2.3 amp pub** (29511699, 🔄 running). Pending: D2.2 sup pub, D2.3 sup pub. Scripts: `run_pub_batch_d2amp_p2.sh` (INTR A), `run_pub_batch_d2amp_p2_resume.sh` (INTR B), `run_pub_batch_d2sup.sh` (INTR C), `run_pub_batch_d2sup_resume.sh` (INTR D). Amp: 2 sessions (38+38 parallel, SIGTERM+resume). Sup: 2 sessions same. |
