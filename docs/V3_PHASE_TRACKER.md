...
| ✅ | NP.T7 (non-propagating complete-even, ξ=0) | 17 | 425 | 850 | 34 | 64 | torsion_gertsenshtein_complete_even.json | ✅ **B.5-post (29705560): amp logZ=+9.93±0.080, sup logZ=−3.28±0.070** (TIMEOUT 1hr, still-active; 1331 dead pts both chains, nlike~105k/112k). **Key result: amp ≈ T7v2 (+10.30) within errors — kinetic ξ contributes only ~0.4 nats. Torsion propagation is NOT necessary for T7-level amplification; the chi1-10 (R̃×∂T) coupling structure alone produces it.** Vs NP.T5 (+10.98, single chi): adding 9 extra chi params reduces logZ slightly via Occam — most of chi1-10 sector is irrelevant to amplification at B₀=0.01. Corner plots: `hpc_results/29705560/np_ceven_{amp,sup}_v1/corner_np_ceven_v1_*.png`. |
...
| B.4a | Stage A amp INTR smoke | ✅ done | 29189966 (8 min INTR). log Z=+9.31±0.13 (vs v2 −0.07; +9.4 nats), ESS=2649/6464. **v2 "null verdict" was an artifact of narrow priors** — v3 reveals joint D_KL=6.59 nats. MAP shifts ~1.1–2.0σ on all 4 params. See `docs/comparison/stage_a_amp_v2_v3.md` |
...
| ~~NP.T7 landscape~~ | ✅ done (29705560, 2026-05-25). amp logZ=+9.93±0.08, sup=−3.28±0.07. Confirms torsion kinetic ξ contributes only ~0.4 nats to T7's amplification; chi1-10 coupling structure alone drives the signal. | — |
...
## Phase E — Localized geometry pivot (DEFERRED, gated on Phase B convergence)
...
| Step | Description | Status | Notes |
| --- | --- | --- | --- |
| E.0 | Dual-Gaussian localized B-field empirical validation | ✅ done (2026-05-16) | `examples/gertsenshtein/theory_e0_dual_gaussian.toml` derived. sigB=5, zc1=25, zc2=75, L=100. A(20)/A(10)=1.00 < 1.05 ✓. P_max=0.00348 at t=18 (CVODE). Modal auto-selects but diverges → GH #367; use `--scheme cvode`. |
| E.1 | Wavepacket IC switch for all v3 campaign scripts | ⏳ deferred | After Phase B convergence |
| E.2 | Localized B-field theory TOMLs for all D1/D2/StageA models | ⏳ deferred | After Phase B convergence |
...
Planned localized-geometry runs (see `docs/PHASE_E_TRACKER.md` for full staging). All reuse the derived E-geometry JSON `torsion_gertsenshtein_general_nonminimal_e_dual_gaussian.json` (E.T5.3, 38 fields, 82 H-terms) unless noted.
...
| E.T5 (E.D2.0) | 5 | Bahamonde sub-theory (β₁₋₃, ξ, δ₁) | ⏳ Stage 5 | Phase B landscape + pub chains complete; localized comparison |
...
| E.T5.3 (E.D2.3) | 9 | Full T5 (β₁₋₃, ξ, δ₁, ζ₁₋₃, χ) | ⏳ Stage 5 | Complete T5; primary localized survey target |
...
- 2026-05-10 — A.0f — HPC jobs 28982006 + 28985879 canceled.
...
- 2026-05-16 — E.0 — Dual-Gaussian localized B-field validated: `theory_e0_dual_gaussian.toml` derived (6 fields, 22 H terms). CVODE smoke tests pass; A(20)/A(10)=1.00 < 1.05 ✓; P_max=0.00348 at t=18. Modal solver auto-selects but diverges (GH #367 filed); workaround: `--scheme cvode`.
...
- 2026-05-25 — B.5-post (NP.T7 done) — 29705560 TIMEOUT 1hr, still-active. **amp logZ=+9.93±0.080**, sup=−3.28±0.070, 1331 dead pts each chain (nlike~105k/112k). **Headline: amp ≈ T7v2 (+10.30) within errors → torsion kinetic propagation (ξ) is NOT necessary for T7-level Gertsenshtein amplification; the chi1-10 R̃×∂T coupling structure alone produces it.** Three-way isolation table — NP.T5 (8D, single chi, +10.98) ≳ T7v2 (18D, chi1-10+ξ, +10.30) ≳ NP.T7 (17D, chi1-10, +9.93): the ~1 nat drop NP.T5→NP.T7 is Occam penalty on 9 extra chi params (most of chi1-10 sector is irrelevant at B₀=0.01); the ~0.4 nat gap NP.T7→T7v2 is ξ's marginal contribution (within combined errors, statistically null). Conclusion: **torsion-EM coupling structure (δ₁+ζ₁₋₃+χ + torsion mass β₁₋₃) is the necessary and sufficient driver of the amplification signal.** Corner plots: `hpc_results/29705560/np_ceven_{amp,sup}_v1/corner_np_ceven_v1_*.png`.
