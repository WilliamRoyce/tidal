...
| **Owned paths** | `docs/cosmology/perturbation_tooling.md` (the memo) · `scripts/research/perturbations/` (install scripts, probes, reproductions) · *extended by the user at planning, 2026-09-15:* `docs/cosmology/rc_planning_record.md` (verbatim plan archive, first commit) · `docs/references.md` — curated rows under `## Cosmology Program` only (**never** regenerate `literature/README.md` in the worktree: the index rebuilds from disk and a worktree holds none of the library) · **additive userbase installs, new directories only:** `Applications/xAct/xPand/` (a version-check shim inside it allowed; diff committed; upstream issue drafted, not filed) and, for the xMAG probe, `Applications/xAct/xMAG/`, `Applications/xAct/xBrauer/`, `Applications/xAct/TraceFree/`, `Applications/SymmetricFunctions/`, `Applications/BrauerAlgebra/` (no patch to any of the xMAG chain) — a before/after sha256 manifest must show nothing else under `Applications/` changed |
