...
| **NOT owned** | `pyproject.toml` (I-532 owns it this wave; the orchestrator settles `wolframclient`'s placement before you start) · `tidalcosmo/{background,spectator,validity}/` · `tidalcosmo/cli/` (the `tidalcosmo derive` command is I-S1B's, which can run it end to end) · any `.wl` or `.wls` · the design docs (do not rewrite or redesign them — but amend a factual error at its instruction site with a dated blockquote, per the flaw protocol) |
