...
> **Status: DECIDED, 2026-09-15.** The user adopted every recommendation: D-A (§2), D-B Option A′
> (§3), the conventions rule (§4) and, after §3.8, a permanent schema-versioned loader — recorded
> as four register rows in `docs/COSMOLOGY_PROGRAM.md`. The lane evidence of §1.1 was re-run by
> the orchestrator from the committed bytes before merge and reproduced (wave board, R-1 row).
...
kernel: the Cobaya-facing modules import no launcher (checked statically, and shown able to fail).
*(Corrected at merge, 2026-09-15: this said "its import closure contains no launcher", but Cobaya
itself imports `subprocess`; the check is — and §3.4 correctly says — of our modules' own imports.)*
