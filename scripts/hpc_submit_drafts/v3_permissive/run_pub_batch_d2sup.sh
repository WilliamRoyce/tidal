#!/usr/bin/env bash
# Publication-pass sup chains for D2.2 and D2.3 (50×ndim nlive).
#
...
#   nohup bash /rds/user/wr286/hpc-work/tidal/scripts/hpc_submit_drafts/v3_permissive/run_pub_batch_d2sup.sh \
#     > /rds/user/wr286/hpc-work/tidal/batch_sup_pub.log 2>&1 &
#
# Strategy: 2-way parallel mpirun (38 ranks each).
#   D2.2 sup pub (8p, nlive=400): est ~57 min at 38 ranks
#   D2.3 sup pub (9p, nlive=450): est ~68 min at 38 ranks (may SIGTERM at 1hr)
#
# If D2.3 sup pub is SIGTERM'd at walltime: pull the _chains/ directory and run
#   uv run python scripts/reconstruct_polychord_inference.py hpc_results/<jobid>/d23_full_sup_v3_pub
# to extract a usable partial posterior from whatever dead points accumulated.
#
...
echo "=== [$(date +%H:%M:%S)] D2.2 sup pub + D2.3 sup pub (38+38 ranks) ==="

# ── D2.2 Shapiro sup pub (8p: β₁₋₃, ξ, δ₁, ζ₁₋₃) ──────────────────────────
...
mpirun -n 38 tidal sample ${DATA} ${PHYS} \
    ...
    --output "${RESULTS_DIR}/d22_shapiro_sup_v3_pub" \
    > "${RESULTS_DIR}/d22_sup_pub.log" 2>&1 &
PID_A=$!

# ── D2.3 Full sup pub (9p: all couplings) ────────────────────────────────────
# shellcheck disable=SC2086
mpirun -n 38 tidal sample ${DATA} ${PHYS} \
    --prior "beta1=arctan_uniform:-89:89" \
    --prior "beta2=arctan_uniform:-89:89" \
    ...
    --output "${RESULTS_DIR}/d23_full_sup_v3_pub" \
    > "${RESULTS_DIR}/d23_sup_pub.log" 2>&1 &
PID_B=$!
...
echo "=== Pull: bash scripts/hpc_shuttle.sh pull ${SLURM_JOB_ID} --src ${RESULTS_DIR} ==="
echo "=== If D2.3 SIGTERM'd: uv run python scripts/reconstruct_polychord_inference.py hpc_results/<jobid>/d23_full_sup_v3_pub ==="
