#!/usr/bin/env bash
# v3 D2.0 Bahamonde sup — sub-theory of T5
#
# Reference v2 sup: 28684410 (log Z = -0.449 ± 0.002, ESS=877; 11 min INTR).
# v3: tachyon-permissive sup chain may surface new structure if propagating-
# torsion couplings stabilise the conversion in regions v2 rejected.

set -euo pipefail

bash scripts/hpc_shuttle.sh submit \
  --template scripts/hpc_templates/polychord_intr.sbatch \
  --name d20_bahamonde_sup_v3 --ntasks 76 \
