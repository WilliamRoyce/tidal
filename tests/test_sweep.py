...
def _make_2d_sweep_results() -> SweepResults:
    ...
    for bp in bpeak_vals:
        for r in r_vals:
            ...
            rows.append({"Bpeak": bp, "R": r, "kappa": 1.0, "P_final": p, "wall_time_s": 1.0})
    return SweepResults(
...
class TestSweepPlotOverlay:
    ...
    def test_render_sweep_1d_with_overlay(self) -> None:
        ...
        results = _make_1d_sweep_results()
        fig, ax = plt.subplots()
        render_sweep_1d(ax, results, "P_final", overlay="sin(kappa * B0 * t_end / 2)**2")

        lines = ax.get_lines()
        assert len(lines) == 2, "Expected 2 lines: numerical (blue) + analytical (orange)"
    ...
    def test_render_sweep_2d_with_overlay_has_three_axes(self) -> None:
        ...
        assert len(fig.axes) >= 3  # noqa: PLR2004 (3 axes + colorbars)
        plt.close(fig)
    ...
    def test_render_sweep_2d_with_overlay_error_panel_near_zero(self) -> None:
        ...
        meshes = [c for c in fig.get_children() for ax in fig.axes for cc in ax.get_children()
    ...
    def test_plot_command_1d_bad_overlay_returns_error(self, tmp_path: Path) -> None:
        ...
        sweep_dir.mkdir()
        (sweep_dir / "sweep.json").write_text(json.dumps({
            "version": 1, "spec_path": None,
            "swept_parameters": {"B0": [0.1, 0.2]},
            "fixed_parameters": {},
            "sim_settings": {"t_end": 50.0},
            "measurements": [],
            "run_dirs": [], "timestamp": "2026-01-01T00:00:00",
            "total_runs": 2, "completed_runs": 2,
        }))
        (sweep_dir / "results.json").write_text(json.dumps({
            "columns": ["B0", "P_final"],
            "rows": [{"B0": 0.1, "P_final": 0.01}, {"B0": 0.2, "P_final": 0.04}],
        }))
