...
This module verifies that the run_with_snapshots utility correctly
captures simulation snapshots at specified intervals, preserves
field structure, and produces equivalent results to manual patterns.
"""
...
class TestRunWithSnapshots:
    """Tests for the run_with_snapshots function."""

    @pytest.fixture
    def grid_1d_snapshot(self) -> CartesianGrid:
        """Create a 1D grid for snapshot tests.

        Returns
        -------
        CartesianGrid
            64-cell grid with bounds (0, 50), periodic.
        """
        return make_grid(
            GridConfig(dim=1, shape=(64,), bounds=((0.0, 50.0),), periodic=True)
        )

    @pytest.fixture
    def grid_1d_small(self) -> CartesianGrid:
        """Create a small 1D grid for fast tests.

        Returns
        -------
        CartesianGrid
            32-cell grid with bounds (0, 20), periodic.
        """
        return make_grid(
    ...
    @pytest.fixture
    def fast_config(self) -> SimulationConfig:
        """Create a fast simulation configuration.

        Returns
        -------
        SimulationConfig
            Configuration with scipy solver and progress disabled.
        """
        return SimulationConfig(
            t_end=10.0, dt=0.1, solver="scipy", backend="numpy", progress=False
        )

    def test_basic(self, grid_1d_snapshot: CartesianGrid) -> None:
        """Verify run_with_snapshots returns storage with correct snapshots."""
        state = gaussian_pulse(
            grid_1d_snapshot, amplitude=1.0, width=2.0, initial_velocity=0.0
        )
    ...
    def test_vs_manual_pattern(self, grid_1d_snapshot: CartesianGrid) -> None:
        """Verify run_with_snapshots gives equivalent results to manual pattern."""
        state1 = gaussian_pulse(
            grid_1d_snapshot, amplitude=1.0, width=2.0, initial_velocity=0.0
        )
        state2 = gaussian_pulse(
            grid_1d_snapshot, amplitude=1.0, width=2.0, initial_velocity=0.0
        )
        pde = KleinGordonPDE(params=KGParameters(mass=0.5))
        config = SimulationConfig(
        ...
        for i in range(-3, 0):
            assert np.allclose(
                storage[i][0].data, snapshots[i][1], atol=1e-5  # type: ignore[index]
            )

    def test_different_intervals(self, grid_1d_small: CartesianGrid) -> None:
        """Verify snapshot intervals are respected."""
        state = gaussian_pulse(grid_1d_small, amplitude=1.0, width=2.0)
        pde = KleinGordonPDE(params=KGParameters(mass=0.5))
        config = SimulationConfig(
        ...
        assert len(storage_coarse) >= min_coarse_snapshots
        assert len(storage_fine) >= min_fine_snapshots

    def test_preserves_field_structure(self, grid_1d_small: CartesianGrid) -> None:
        """Verify storage preserves FieldCollection structure."""
        state = gaussian_pulse(grid_1d_small, amplitude=1.0, width=2.0)
        pde = KleinGordonPDE(params=KGParameters(mass=0.5))
        config = SimulationConfig(
    ...
    def test_2d_grid(self) -> None:
        """Verify run_with_snapshots works with 2D grids."""
        # Need a centered grid for this test
        grid = make_grid(
            GridConfig(
                dim=2,
                shape=(32, 32),
