...
class TestWlsGeneration:
    def test_no_perturbation_section_omits_metadata_keys(
        self,
        gertsenshtein_config: dict[str, Any],
    ) -> None:
        ...
        # The v5 JLM symbolic reducer must remain absent: no `PerturbativeReduce[`
        # call (its public function name) and no `JLMReduce` reference.  The
        # `PerturbativeReduction.wl` filename is reused by the v6 LPS module
        # (Lagrangian Perturbative Substitution), which is a different algorithm
        # that ships with v6 and is loaded universally — but its `ReduceLagrangian`
        # call is gated on `[perturbation]` being present, so a non-perturbative
        # theory like gertsenshtein never invokes it.
        assert "PerturbativeReduce[" not in wls
        assert "JLMReduce" not in wls
        assert "ReduceLagrangian[" not in wls
