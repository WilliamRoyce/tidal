...
def _transform_term(
    ...
    if "coordinate_dependent" in result:
        result["coordinate_dependent"] = _remap_coord_deps(
            result["coordinate_dependent"], propagation_axis, killed,
            f"term operator={op}",
        )
...
def _reduce_equations(
    ...
    original_fields: set[str] = {
        eq["field"] for eq in result.get("equations", [])
    }
    ...
    for eq in result.get("equations", []):
        ...
        new_terms = [
            t for term in terms
            if (t := _transform_term(term, remap, propagation_axis, killed))
            is not None
        ]
        if not new_terms:
            continue
    ...
    result["fields"] = [
        f for f in result.get("fields", []) if f["name"] in surviving
    ]
...
def _reduce_hamiltonian(
    ...
    for hterm in canonical["hamiltonian_terms"]:
        ...
        transformed = _transform_hamiltonian_term(
            hterm, remap, propagation_axis, killed,
        )
...
def _reduce_coupling_matrices(
    result: dict[str, Any],
    spec_data: dict[str, Any],
    surviving_fields: set[str],
) -> None:
    """Filter coupling matrices to surviving fields."""
    ...
    for matrix_key in (
        "mass_matrix", "coupling_matrix",
        "mass_matrix_symbolic", "coupling_matrix_symbolic",
    ):
        ...
        for i in surviving_indices:
            if i < len(matrix):
                row = matrix[i]
                new_row: list[Any] = [
                    row[j] for j in surviving_indices if j < len(row)
                ]
...
def reduce_spec(
    ...
    surviving_fields, eliminated_fields = _reduce_equations(
        result, remap, propagation_axis, killed,
    )
    ...
    _reduce_coupling_matrices(result, spec_data, surviving_fields)
