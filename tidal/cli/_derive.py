...
def _wls_canonical_phase_a(ctx: _WlsContext, all_heads_str: str) -> list[str]:  # noqa: C901, PLR0914, PLR0915
    ...
    lines.extend(
        [
            ...
            "];",
            "",
            "(* Hamiltonian sector filter: lagComp is ONLY used for the Hamiltonian *)",
            "(* (EOM come from VarD on the abstract L^(2), not from lagComp).      *)",
            "(* Drop terms containing torsion fields — they contribute only        *)",
            "(* torsion self-energy or interaction terms that are excluded from     *)",
            "(* the GW+EM Hamiltonian anyway.                                      *)",
            "(* NOTE: Keep background (Abar) terms — these include F-bar^2 = B0^2  *)",
            "(* graviton effective mass, which IS a GW self-energy contribution.   *)",
            "If[TrueQ[$tidalHamiltonianFilter],",
            "  Module[{nBefore = Length[lagTerms], hamTerms},",
            f"    hamTerms = Select[lagTerms, FreeQ[#, {ctx.prefix}T] &];",
            "    lagTerms = hamTerms;",
            '    Print["  Hamiltonian sector filter on lagComp: ", nBefore, " -> ",',
            '      Length[lagTerms], " terms (torsion sectors dropped)"];',
            "  ]",
            "];",
            "",
...
def _wls_canonical_phase_b(ctx: _WlsContext, _all_heads_str: str) -> list[str]:
    ...
    lines.extend(
        [
            ...
            "    (* Skip constraint fields (no velocity in their own equation) *)",
            "    feq = Select[fieldEquations, #[[1]] === cn &];",
            "    isDynamic = Length[feq] > 0 && !FreeQ[feq[[1, 2]], Derivative[Sequence @@ velOrders][compToFunc[cn]]];",
            "    isDynamic",
            "  ]",
            "]];",
