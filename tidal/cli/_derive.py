...
def _wls_precompute_cd_component_values(
    ...
    lines: list[str] = [
        ...
        "",
        "(* Helper: pre-compute and assign ComponentValues for a tensor at given    *)",
        "(* placement. Uses StaggeredToBasis to compute, then assigns via          *)",
        "(* ComponentValue[ComponentArray[...], computed].                          *)",
        "(* basisIdxs: list of basis-indexed slots, e.g. {-{a,chart}, -{b,chart}} *)",
        "(* sourceExpr: the abstract expression to compute components from.        *)",
        "(* When sourceExpr is Null, compute from the tensor's own raised form.    *)",
        "tidalPrecomputeCV[tensor_, basisIdxs_List, sourceExpr_, chart2_, christoffels_] :=",
        "  Module[{comp},",
        "    If[sourceExpr =!= Null,",
        "      comp = StaggeredToBasis[sourceExpr, chart2, christoffels],",
        "      comp = StaggeredToBasis[tensor @@ basisIdxs, chart2, christoffels]",
        "    ];",
        "    Block[{Print = Null},",
        "      ComponentValue[ComponentArray[tensor @@ basisIdxs], comp]",
        "    ];",
        "    comp",
        "  ];",
        "",
    ...
    # For each field, pre-compute CD1 through CD4 ComponentValues
    for df in dyn_fields:
        head = df["head"]
        field_name = df["name"]

        for cd_level in range(1, 5):
            ...
            lines.extend(
                [
                    ...
                    "  Module[{slots, prevSlots, nSlots, downIdxs, upIdxs, "
                    "naturalDown, prevDownIdxs, cdExpr},",
                    f"    slots = SlotsOfTensor[{cd_head}];",
                    "    nSlots = Length[slots];",
                    f"    prevSlots = SlotsOfTensor[{prev_head}];",
                    "",
                    "    (* Build basis index lists using the manifold's abstract indices *)",
                    "    (* Natural-down: every slot gets -{idx, chart} form *)",
                    f"    downIdxs = Table[-{{IndexRange[a, z][[i]], {chart}}}, {{i, nSlots}}];",
                    "    (* All-up: every slot gets {idx, chart} form *)",
                    f"    upIdxs = Table[{{IndexRange[a, z][[i]], {chart}}}, {{i, nSlots}}];",
                    "",
                    "    (* Compute natural-down from CD[-a]@prev_tensor[down_indices] *)",
                    f"    prevDownIdxs = Table[-DummyIn[Tangent{manifold}], {{Length[prevSlots]}}];",
                    f"    cdExpr = {cd}[-a][{prev_head} @@ prevDownIdxs];",
                    f"    naturalDown = tidalPrecomputeCV[{cd_head}, downIdxs, cdExpr, {chart}, False];",
                    "",
                    "    (* Compute all-up by raising (uses metric ComponentValues) *)",
                    f"    tidalPrecomputeCV[{cd_head}, upIdxs, Null, {chart}, False];",
                    "",
                    f'    Print["  {cd_head}: ", nSlots, " indices, ComponentValues assigned"];',
                    "  ],",
    ...
    *,
    ...
    if has_metric_pert:
        lines.extend(
            [
                ...
                "(* Expand eagerly: propagate zeros before next substitution step *)",
                "l2Raw = Expand[l2Raw];",
    ...
    if ctx.torsion:
        ...
        lines.extend(
            [
                ...
                "(* Expand eagerly: propagate torsion zeros before curvature zeroing *)",
                "l2Raw = Expand[l2Raw];",
    ...
    lines.extend(
        [
            ...
            "(* Expand eagerly: propagate curvature zeros before contortion zeroing *)",
            "l2Raw = Expand[l2Raw];",
    ...
    lines.extend(
        [
            "",
            "(* === Per-term adaptive canonicalization ===                             *)",
            "(* ToCanonical + ContractMetric scale super-linearly with expression     *)",
            "(* size (467s on full non-minimal L^(2)). Per-term processing is O(N)   *)",
            "(* with adaptive batch sizing based on measured first-batch cost.        *)",
            "(* Ref: issue #201 — L^(2) cleanup bottleneck for multi-field theories. *)",
            "Module[{l2Terms, nTerms, canonBatch, canonBatchSize, tFirstBatch,",
            "         firstBatchTime, canonResult, batchStart, batchEnd},",
            "  l2Terms = If[Head[l2Raw] === Plus, List @@ l2Raw, {l2Raw}];",
            "  nTerms = Length[l2Terms];",
            '  Print["Canonicalizing ", nTerms, " L^(2) terms (per-term adaptive)..."];',
            "",
            "  (* Measure first batch to adapt batch size *)",
            ...
            "",
            "  (* Adaptive: scale batch size to target ~2s per batch *)",
...
def _wls_canonical_phase_a(ctx: _WlsContext, all_heads_str: str) -> list[str]:  # noqa: PLR0914
    ...
    lines: list[str] = [
        ...
        "  lagForCanon = ToCanonical[lagForCanon];",
        f"  lagForCanon = ContractMetric[lagForCanon, {ctx.metric}];",
