...
def _wls_torsion_curvature_decomposition(ctx: _WlsContext) -> list[str]:
    ...
    **This decomposition is an exact identity** (not an approximation).  It
    ...
    return [
        ...
        "];",
        "",
        "(* === Epsilon (Levi-Civita) pre-evaluation ===                          *)",
        "(* Parity-odd Lagrangians contain epsilon tensors that survive the R̃     *)",
        "(* decomposition.  If left symbolic, they cause O(dim^16) blowup in     *)",
        "(* TraceBasisDummy during component decomposition (issue #246).           *)",
        "(*                                                                        *)",
        "(* Strategy: apply ToBasis + EvaluateEpsilonComponents to each epsilon-  *)",
        "(* containing term INDIVIDUALLY (not to the full sum), then recombine.   *)",
        "(* This converts each ε to ±1 at the Lagrangian level, BEFORE xPert     *)",
        "(* expansion — preventing the term-count explosion in L^(2).             *)",
        "If[!FreeQ[lOriginal, _?IsEpsilonTensor],",
        '  Print["Epsilon pre-evaluation: detected Levi-Civita tensor in Lagrangian"];',
        "  Module[{epsTerms, nonEpsTerms, epsEval, tEps = AbsoluteTime[]},",
        "    If[Head[lOriginal] === Plus,",
        "      epsTerms = Select[List @@ lOriginal, !FreeQ[#, _?IsEpsilonTensor]&];",
        "      nonEpsTerms = Select[List @@ lOriginal, FreeQ[#, _?IsEpsilonTensor]&];",
        "    ,",
        "      epsTerms = {lOriginal}; nonEpsTerms = {};",
        "    ];",
        '    Print["  epsilon terms: ", Length[epsTerms], ", non-epsilon: ", Length[nonEpsTerms]];',
        "    (* Per-term ToBasis + epsilon eval + TraceBasisDummy *)",
        "    Off[Validate::repeated];",
        "    epsEval = Total[Map[",
        "      Function[term,",
        "        Module[{e = term},",
        f"          e = ToBasis[{ctx.chart}][e];",
        f"          e = EvaluateEpsilonComponents[e, {ctx.chart}];",
        "          e = Expand[e];",
        "          e = TraceBasisDummy[e];",
        "          e = Expand[e];",
        "          e",
        ...
        "    lOriginal = Total[nonEpsTerms] + epsEval;",
        "    lOriginal = Expand[lOriginal];",
        '    Print["  epsilon evaluation complete: ", If[Head[lOriginal]===Plus, Length[lOriginal], 1], ',
        '      " terms (", Round[AbsoluteTime[] - tEps, 0.1], "s)"];',
        "  ];",
        "];",
        "",
