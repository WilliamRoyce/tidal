...
def _wls_canonical_from_eom(ctx: _WlsContext) -> list[str]:
    ...
    return [
        ...
        "canonicalFieldRates = <||>;",
        "Do[",
        "  Module[{idx, compName, piName},",
        "    idx = dynIndices[[i]];",
        "    compName = allCompNames[[idx]];",
        "    piName = compName;  (* pi_field_name convention *)",
        '    canonicalFieldRates[compName] = {<|"coefficient" -> 1.0,',
        '      "operator" -> "identity",',
        '      "field" -> "pi_" <> piName|>};',
        '    Print["Field rate ", compName, ": identity(pi_", piName, ")"];',
        "  ],",
        "  {i, nDyn}",
        "];",
        "",
        ...
        "(* constraint_momenta empty: fast path assumes K=I, no mixed *)",
        "(* time-space derivatives, so pi(constraint) = 0. *)",
    ...
    **EOM-based fast path** (component count > threshold): Constructs the
    ...
    lines.extend(
        [
            ...
            "          If[kInvEntry =!= 0,",
            "            piName = allCompNames[[dynIndices[[j]]]];  (* field name for pi *)",
            ...
            "",
            "(* Print constraint field diagnostics *)",
            "(* Extract constraint canonical momenta: pi(q_i) = dL/d(dt q_i) *)",
            "(* for fields with time_derivative_order == 0.  Exported for *)",
            "(* diagnostics — the Legendre transform analytically cancels *)",
            "(* pi(q)*dt(q) so these do not appear in the Hamiltonian. *)",
            "constraintMomentaTerms = <||>;",
            "Do[",
            "  Module[{compName, piComp, piTerms},",
            "    compName = allCompNames[[k]];",
            "    If[!KeyExistsQ[canonicalFieldRates, compName],",
            '      Print["Field rate ", compName, ": (constraint - no rate)"];',
            "      piComp = piCompList[[k, 2]];",
            "      If[piComp =!= 0,",
            "        piTerms = ParseMultiFieldRHS[piComp, compName, allCompNames];",
            "        constraintMomentaTerms[compName] = piTerms;",
            '        Print["  Constraint momentum pi(", compName, "): ", piComp];',
            "      ,",
            '        Print["  Constraint momentum pi(", compName, "): 0"]',
            "      ]",
            "    ]",
            "  ],",
            "  {k, Length[allCompNames]}",
            ...
            '  "constraint_momenta" -> constraintMomentaTerms,',
