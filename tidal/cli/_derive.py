...
def _wls_canonical_phase_a(ctx: _WlsContext, all_heads_str: str) -> list[str]:  # noqa: PLR0914
    ...
    lines.extend(
        [
            ...
            "(* Per-term splitting was a workaround for the old single-pass ToBasis  *)",
            "(* and is counterproductive: 34 separate passes are slower than 1.      *)",
...
def _wls_canonical_phase_b(_ctx: _WlsContext, _all_heads_str: str) -> list[str]:
    ...
    lines.extend(
        [
            ...
            "(* terms).                                                               *)",
            "(* isPureTimeDerivGE2: detect Derivative[n,0,...,0][f][args] with n>=2.   *)",
            "(* Head structure: Head[Derivative[n,...][f][args]] = Derivative[n,...][f], *)",
            "(*   Head[Derivative[n,...][f]] = Derivative[n,...],                       *)",
            "(*   Head[Derivative[n,...]] = Derivative.                                 *)",
            "(* So we need Head[Head[Head[expr]]] === Derivative.                       *)",
            "isPureTimeDerivGE2[expr_] := Module[{h = Head[expr], dOp, orders},",
