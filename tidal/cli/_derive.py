...
def _wls_constraint_elimination(
    ...
    **Smart depField selection** (avoids 1/parameter singularities):
    ...
    return [
        ...
        "    (* EXCLUDE fields that have algebraic solutions (depFields) —     *)",
        "    (* those must be SUBSTITUTED via algRules, not zeroed.  A field   *)",
        "    (* can be both gradient-zero (Phase 1: TT transverse constraint   *)",
        "    (* replaces its dynamical eq) and an algebraicDepField (Phase 2:  *)",
        "    (* solved from another field's degenerate constraint).  Zeroing   *)",
        "    (* it would lose coupling terms that the algebraic solution       *)",
        "    (* transfers to surviving fields (e.g. Gertsenshtein h4->a2).     *)",
        "    Module[{depFieldNames = Keys[algebraicDepFields]},",
        "      zeroRules = Flatten[Table[",
        "        {compToFunc[name][___] -> 0,",
        "         Derivative[__][compToFunc[name]][___] -> 0},",
        "        {name, Select[Join[gradZeroFields, degenerateEqFields],",
        "           !MemberQ[depFieldNames, #] &]}",
        "      ]]",
        "    ];",
        "",
