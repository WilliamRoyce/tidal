...
def _wls_multi_field_eom(  # noqa: PLR0912, PLR0914, C901, PLR0915
    ...
    lines.extend(
        [
            "(* Re-enable Validate::inhom after VarD is done *)",
            "On[Validate::inhom];",
    ...
    # placements. For flat Minkowski, uses direct sign correction
    # (raising/lowering = multiply by η_nn = ±1). For curved metrics,
    ...
    # --- Pre-compute CD4 shorthand ComponentValues (NATURAL PLACEMENT ONLY) ---
    # For flat Minkowski: SeparateMetric handles index raising via explicit eta,
    # so ToValues only needs the natural (all-downstairs) placement.
    # This avoids the 2^6=64 placement explosion that timed out at 600s.
    for df in dyn_fields:
        head = df["head"]
        cd3_head = f"CD3{head}"
        cd4_head = f"CD4{head}"
        lines.extend(
            [
                ...
                f"    Module[{{freshIdx = DummyIn[Tangent{ctx.manifold}],",
                f"            cd3Idxs = Table[DummyIn[Tangent{ctx.manifold}], {{n, Length[SlotsOfTensor[{cd3_head}]]}}]}},",
                f"      Module[{{cd3Expr = {cd3_head} @@ MapThread[If[#1 === 1, #2, -#2] &, {{SlotsOfTensor[{cd3_head}], cd3Idxs}}]}},",
