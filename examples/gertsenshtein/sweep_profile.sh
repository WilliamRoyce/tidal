...
#
# Sweeps B0_peak (peak field strength) and R (Gaussian half-width) and measures
...
#   P = sin^2(kappa * B0_peak * R * sqrt(pi/2))
#     = sin^2(kappa * integral(B_x(z), -inf, +inf) / 2)
#
# where B_x(z) = B0_peak * exp(-z^2 / (2*R^2)) and integral = B0_peak * R * sqrt(2*pi).
#
# Parameter ranges:
#   B0_peak in [0.02, 0.20] (8 points) — scan from weak to moderate coupling
#   R in [2.0, 15.0] (6 points)        — scan from narrow to wide profiles
#
# For kappa=1, the argument kappa*B0_peak*R*sqrt(pi/2) ranges from
...
tidal plot ../data/gertsenshtein_sweep_profile --type sweep \
  --metric P_final \
  --title "Gertsenshtein P(B0_peak, R) — Boccaletti formula validation" \
