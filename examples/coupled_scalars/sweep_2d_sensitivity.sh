...
#
# Runs a Latin Hypercube sweep over gCpl × mChi2 and performs Morris
# sensitivity screening to determine which parameter controls conversion.
# Generates advanced visualizations: parallel coordinates, tornado,
# and scatter matrix.
#
# Note: Morris screening works with any sampling strategy (grid, LHS, etc.).
# Sobol analysis requires Saltelli-structured sampling (not yet supported).
#
# Demonstrates features F2b (LHS), F4 (Morris), F5 (query), F8 (viz).
#
# Running this script:
...
echo ""
echo "=== Sensitivity Analysis (Morris screening) ==="
