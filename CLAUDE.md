...
- **FV ↔ TorsionCDT dark-photon equivalence (sign-flip!)**: The CDT I3 invariant writes `+α₃·V·V` (positive prefactor), while FV Proca writes `-½ mT²·t·t` (negative prefactor). Under mostly-plus metric signature, the physical mass squared in the spatial trace sector is `m² = -2·α₃` for CDT and `m² = +mT²` for FV, so the equivalence map is `mT2 = -2·alpha3` (sign flip, NOT `mT2 = 2·alpha3`). CDT `α₃ < 0` ↔ FV `mT2 > 0` is the stable Proca regime. This was a subtle docs bug that caused a spurious 1.2% "tension" in an earlier cross-check (Issue #318, resolved 2026-04-24). With the correct mapping, FV and CDT simulation P_max agree to ~2.8e-14 (double-precision epsilon).
