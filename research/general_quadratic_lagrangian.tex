...
\begin{tabular}{@{}llcll@{}}
\toprule
\textbf{Sector} & \textbf{Count} & \textbf{Mixing type} & \textbf{Mechanism} & \textbf{Mass dim} \\
\midrule
$\Rt \times F^2$ & 4 & $h \leftrightarrow f$ & One $F \to \bar{F}$ & $[M^6]$, suppressed \\
$\Rt^2 \times F$ & 8 & $h$ self-int.\ mod $B_0$ & One $F \to \bar{F}$ & $[M^6]$, suppressed \\
$T^2 \times F$ & 3 & $t$ mass corr.\ $\propto B_0$ & Both $T \to \epsilon t$ & $[M^4]$, \textbf{unsuppressed} \\
$\del T \times F^2$ & 5 & $t \leftrightarrow f$ & One $F \to \bar{F}$ & $[M^6]$, suppressed \\
\bottomrule
\end{tabular}

\paragraph{EFT mass-dimension classification.}
The ``cubic'' sectors split by mass dimension: $T^2 \times F$ has
Lagrangian dimension~$[M^4]$ (since $[T] = [M^1]$) and requires a
\textbf{dimensionless} coupling --- it is on equal footing with the
quadratic sector and should be included in parameter sweeps.  All other
cubic sectors ($\Rt \times F^2$, $\Rt^2 \times F$, $\del T \times F^2$)
have dimension~$[M^6]$ and are suppressed by $1/\Lambda^2$ where
$\Lambda \sim \Lambda_4 = (M_\mathrm{Pl}\, m^3)^{1/4}$ is the PGT
cutoff~\cite{aoki2020ghostfree}.  See
\texttt{docs/tex/background\_validity.tex}, \S\ref{sec:bv-eft} for the
full EFT analysis.
\end{table}
