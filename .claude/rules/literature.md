...
1. Check `literature/` first — 140+ arXiv TeX sources stored locally (but never limit
   research to local papers). `literature/README.md` is the full generated inventory
   (which directory holds what, and which file to open); `docs/references.md` is the
   curated index saying *why* each paper is there.
...
3. Key papers available locally:
   ...
   - Cosmology program: see the "Cosmology Program" section of `docs/references.md`
     (CMB canon, WS3 oscillatory solvers, WS6 polology, O4 birefringence)
4. For new papers: `uv run python -m scripts.bibaudit.fetch_fulltext <arxiv-id> ...`
   (extracts to `literature/<id>/`; old-style ids use `_` for `/`)
5. Then regenerate the inventory —
   `uv run python -m scripts.bibaudit.index_literature` — and add a curated line to
   `docs/references.md` saying why the paper is in the library
